                ...
                if( ns ==="@directives" ){
                    ...
                    if( directives ){
                        ...
                        if( newContext.includes(dName) ){
                            ...
                            const dName = this.name.value().toLowerCase();
                            this.scope.isForContext = dName==='for' || dName==='each';
            ...
            if( this.value && this.value.isJSXExpressionContainer ){
                ...
                if( item ){
                    ...
                    if(makeType){
                        ...
                        if(makeType === refType && Utils.isStack(desc)){
                            let _desc = desc.description();
                            if(Utils.isStack(_desc)){
                                const result = this.getIteratorResultType(makeType, _desc.getContext());
                                if(result){
                                    makeType = result;
                                }
                            }
                        ...
                        segments.forEach( (stack,index)=>{
                            (function(stack, _type){
                                ...
                                stack.definition=function definition(ctx){
                                    return {
                                        text:`(local var) ${this.value()}:${this.type().toString(ctx|this.getContext())}`,
                                        location:this.getLocation(),
                                        file:this.compilation.file,
                                    }
                                }
                                stack.hover=function hover(ctx){
                                    return {
                                        text:`(local var) ${this.value()}:${this.type().toString(ctx|this.getContext())}`
                                    }
                                }
        ...
        return this.getAttribute('parserSlotScopeParamsStack',()=>{
            if(!this.value.isLiteral){
                ...
                if(value.isIdentifier){
                    return new Declarator(this.compilation, value.node, this.scope, this.node, this);
                }else{
