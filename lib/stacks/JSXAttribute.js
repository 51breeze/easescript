...
class JSXAttribute extends Stack{
    ...
    definition( context ){
        ...
        if( this.isAttributeSlot && this.value ){
            ...
            if( this.jsxElement.isSlotDeclared ){
                ...
                let type = this.getGlobalTypeById('any');
                let stack = key;
                if( desc ){
                ...
                if( this.jsxElement.isSlotDeclared ){
                    ...
                    if( desc ){
                        if( desc.isJSXElement ){
                            ...
                            const attr = attributes.length ===1 ? attributes[0] : attributes.find( attr=>attr.name.value() === key );
                            if( attr && attr.isJSXAttribute ){
                                return attr.description();
                            } 
                        }else if( desc.isAnnotation && desc.args && desc.args.length > 0 ){
                            const key = this.name.value();
                            const argument = desc.args.length === 1 ? desc.args[0] : desc.args.find( arg=>arg.name===key );
                            if( argument ){
    ...
    type(){
        if( this.jsxElement.isSlot ){
            ...
            const desc = this.getSlotAttrDescription();
            if( desc ){
                if( desc.isJSXElement ){
                    const attributes = desc.openingElement ? desc.openingElement.attributes : [];
    ...
    async parserDirective(){
        ...
        if( name ==="if" || name ==="elseif" ){
            ...
            if( !this.value || (this.value.isLiteral && !this.value.value().trim()) ){
                ...
                desc = await this.parserAttributeValueStack();
            ...
            if( this.value && this.value.isJSXExpressionContainer ){
                ...
                if( divide ){
                    ...
                    item = await this.parserAttributeValueStack(this.compilation.source.substr(0,startAt+divide.index), startAt);
                    desc = await this.parserAttributeValueStack(this.compilation.source.substr(0,startAt+value.length), startAt + divide.index + divide[0].length );
                }else{
                    if( name ==="each" ){
    ...
    async parserAttributeValueStack( content, startAt = 0, isDeclarator ){
        ...
        const createStack = async (expression,startAt) =>{
        ...
        if( content ){
            return await createStack( content , startAt);
        }else if( this.value ){
            if( this.value.isJSXExpressionContainer ){
                ...
                return this._attributeValueStack = await createStack(this.compilation.source.substr(0, startAt+len),  startAt);
            }
        }
        return null;
    }
    ...
    async parser(){ 
        return await this.callParser(async ()=>{
            ...
            if( this.jsxElement.isDirective){
                ...
                if( direName !=="custom" ){
                    ...
                    if( ['condition','name','item','key','index'].includes( name ) ){
                        const desc = await this.parserAttributeValueStack();
            ...
            if( this.isAttributeDirective ){
                await this.parserDirective();
            }else if( this.isAttributeBinding ){
                if( this.value && !this.value.isJSXExpressionContainer ){
                    await this.parserAttributeValueStack();
                }
            }else{
                const nsStack = this.getNamespaceStack();
                ...
                if( this.isAttributeXmlns ){
                    ...
                    if( namespace ){
                        if( namespace.includes('::') ){
                            ...
                            if( _ns ==="@events"){
                                const defineClassModule = await this.loadTypeAsync( defineClass );
                                namespace = _ns;
