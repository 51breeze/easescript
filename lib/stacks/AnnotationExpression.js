                    ...
                    if( desc ){
                        ...
                        if( value ){
                            ...
                            if(value.isUnionType){
                                generics = value.elements.filter(el=>el.type().isClassGenericType).map(el=>el.type())
                            }else if(value.isClassGenericType){
                                generics = [value];
                            }else{
                                types.push(value)
                            }
                            ...
                            if(generics){
                                generics.forEach( type=>{
                                    types.push(...type.types.map(el=>el.type()))
                                })
                            }
                            ...
                            if(value.isUnionType){
                                value.elements.forEach(el=>{
                                    const type = el.type();
                                    if(generics && generics.includes(type)){
                                        return;
                                    }else{
                                        types.push(type)
                                    }
                            ...
                            types.forEach(type=>merge.add(type))
                            value = ctx.apply(merge.type());
        ...
        if( isServer ){
            ...
            if( args.length != 1 ){
                ...
                args.forEach( item=>{
                    ...
                    let file = flag ? item.value.substr(1) : this.compiler.resolveManager.resovleAssets(item.value, this.file);
                    if( !file ){
