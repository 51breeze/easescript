...
class ExportNamedDeclaration extends Stack {
    constructor(compilation,node,scope,parentNode,parentStack){ 
        ...
        if(this.declaration && this.parentStack.isModuleDeclaration){
            ...
            if(decl.isDeclaratorFunction || decl.isDeclaratorVariable || decl.isNamespaceDeclaration){
                ...
                if(decl.isNamespaceDeclaration){
                    module.namespaces.set(decl.value(), decl.module)
                }
                ...
                module.setType(decl.value(), decl);
            }
        }
    }
    ...
    getAllExportDescriptors(exclude=null){
        ...
        if( this.declaration ){
            ...
            }else if(decl.isDeclaratorDeclaration || decl.isInterfaceDeclaration){
    ...
    description(){
        const decl = this.declaration;
        if(decl){
            if(decl.isClassDeclaration || decl.isDeclaratorDeclaration){
                return decl.module;
            }else if( decl.isFunctionDeclaration ){
                return decl;
            }else if( decl.isAssignmentExpression ){
