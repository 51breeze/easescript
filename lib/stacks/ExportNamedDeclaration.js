...
class ExportNamedDeclaration extends Stack {
    ...
    async getResolveCompilation(){
        ...
        if( this.compiler.options.suffix === this.getFileExt() ){
            const compilation = this.resolveCompilation = await this.compilation.createChildCompilation(this.getResolveFile(), this.compilation.file, null);
            if( !compilation ){
    ...
    async parser(){ 
        return await this.callParser(async ()=>{ 
            this.declaration && await this.declaration.parser();
            this.allSettled(this.specifiers, async(item)=>await item.parser())
            const resolve = this.getResolveFile();
            if(resolve){
