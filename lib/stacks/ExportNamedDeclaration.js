...
class ExportNamedDeclaration extends Stack {
    ...
    async createCompleted(){
        ...
        if(compilation){
            ...
            if(this.parentStack.isPackageDeclaration && this.specifiers.length>0){
                const jsModule = this.getResolveJSModule();
                if(jsModule){
                    this.specifiers.forEach(item=>{
                            ...
                            value.isDeclaratorFunction )
                        {
                            if(JSModule.is(value.module)){
                                value.module.descriptors.get(key).forEach((value, key)=>{
                            ...
                            value.isDeclaratorVariable ||
                            value.isDeclaratorFunction ||
                            value.isDeclaratorTypeAlias ||
