...
class ClassDeclaration extends Stack{
    ...
    async createCompleted(){
        ...
        await Promise.allSettled(this.imports.map((stack)=>stack.addImport(this.module, this.parentStack.scope)));
        if( this.inherit ){
        ...
        await Promise.allSettled(this.implements.map(stack=>loadType(stack)));
        ...
        const check = (left,right)=>{
            ...
            if( left.isMethodDefinition ){
                if( !right.isMethodDefinition ){
                    ...
                    if( lType && rType && !Utils.checkTypeForBoth(lType.type(),rType.type())){
                ...
                const result = left.params.every( (item,index)=>{
                    ...
                    return rType ? Utils.checkTypeForBoth(item.type(), rType ) : false;
                });
                if( !result ){
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            await this.allSettled(this.usings, async(stack)=>{
                await stack.parser();
            })

            if( this.genericity ){
                await this.genericity.parser();
            ...
            await this.allSettled(this.metatypes, async(stack)=>{
                await stack.parser();
            })

            await this.allSettled(this.annotations, async(stack)=>{
                await stack.parser();
            });

            await this.allSettled(this.implements, async(stack)=>{
                const impModule = await this.loadTypeAsync(stack.value());
                if( impModule ){
            ...
            await this.allSettled(this.body, async(stack)=>{
                await stack.parser();
            });
        })
    }
