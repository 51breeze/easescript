...
class ClassDeclaration extends Stack{
    constructor(compilation,node,scope,parentNode,parentStack){
        ...
        const module = this.module = compilation.createModule(
            this.namespace,
            this.id.value(),
        ...
        try{
                ...
                let push = (module, stack)=>{
                    if( !module ){
                        ...
                        if( Utils.checkDepend(self,module) ){
                            ...
                            self.extends = module;
                            module.used = true;
                            if(!module.children.some(child=>Utils.isEqualModule(child, self))){
            ...
            const pushImp = (module, stack)=>{
                if(module && self !== module){
                    if(!(module.isInterface || module.isStructTable)){
    ...
    implementCheck(interfaceModule){
        ...
        const check = (left,right)=>{
            if(!left)return;
            if(Utils.isModifierPrivate(left))return;
            if(Utils.isModifierPrivate(right))return;
            ...
            if(!right){
                ...
                return (impStack||this.id).error(1032,left.value(),type,interfaceModule.getName(),this.module.getName());
            }
            if( left.isMethodDefinition ){
                if(!right.isMethodDefinition){
                    let result = !!left.isMethodGetterDefinition;
                    if(right.isPropertyDefinition){
                        if(left.isMethodSetterDefinition){
                            result = !right.isReadonly;
                        }else if(!right.isReadonly){
                            result = !!left.module.getMember(left.value(), 'set', true)
                        }
                    }
                ...
                const result = left.params.every( (item,index)=>{
                    ...
                    return rType ? Utils.checkTypeForBoth(item.type(), rType, false , ctx) : false;
                });
                if( !result ){
