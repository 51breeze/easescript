...
class CallExpression extends Expression{
    ...
    descriptor(){
        return this.getAttribute('CallExpression.descriptor', ()=>{
            ...
            const fetch = (type, exclude=null, noFindModule=false)=>{
                ...
                if(type.isTupleType || type.isLiteralArrayType)return null;
                if(type.isComputeType){
                    ctx = ctx || this.callee.getContext();
                    type = type.getComputeResult(null, ctx);
                }
                ...
                if(type.isTypeofType){
                    ...
                    return fetch(type.left.type(), type, true) || fetch(type.right.type(), type, true);
                }else if(type.isUnionType){
                    const els = type.elements;
                    for(let index=0; index<els.length;index++){
                        let res = fetch(els[index].type(), type, true)
                        if(res)return res;
                    }
                    return null;
    ...
    presetPredicateType(declareParams, args){
        ...
        if(hasUnary && !isLogicalFlag)return;
        ...
        if(pp.isIfStatement || pp.isConditionalExpression){
            ...
            if(scope && scope.allowInsertionPredicate()){
                ...
                if(isLogicalFlag && !isLogicalTrueFlag){
                    ...
                    if(argumentType.isUnionType){
                        const matched = argumentType.elements.filter(item=>!constraint.is(item.type(), ctx));
                        if(matched.length>0){
                            if(matched.length>1){
                    ...
                    const inference = (type)=>{
                        ...
                        return ctx.infer(type) || anyType || (anyType=Namespace.globals.get('any'));
                    }
            ...
            let assignType = this.getContext().apply(type.inferType()); 
            let rDesc = type.value.description();
    ...
    parser(){
        ...
        if( !description ){
            ...
            if(description.module === this.module && description.isMethodDefinition){
                const context = this.parentStack.getParentStack(parent=>parent.isMethodDefinition);
                if(context === description){
                    context.addRecursionCallStacks(this)
                }
            }else if(this.callee.isIdentifier){
                const desc = this.description()
                if(this.is(desc) && desc.isVariableDeclarator && desc.value() === this.callee.value()){
                    const context = this.parentStack.getParentStack(parent=>{
                        if(parent.isMethodDefinition)return true;
