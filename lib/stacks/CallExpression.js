...
class CallExpression extends Expression{
        ...
        return this.getAttribute('CallExpression.type',()=>{
            ...
            if(description){
                ...
                if(type){
                    ...
                    const context = this.getContext();
                    const final = context.apply(type);
                    context.make(final);
                    return final;
                }
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            if( description && (description.isMethodDefinition || description.isFunctionExpression) ){
            ...
            if( !hasRest ){
                if( requireCount > 0 && argsLength < requireCount || length < argsLength ){
                    const callee = this.callee.isMemberExpression ? this.callee.property : this.callee;
                    callee.error(1000,requireCount,argsLength);
                }
            }
        })
    }
