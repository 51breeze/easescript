...
class CallExpression extends Expression{
    ...
    definition(){
        ...
        if(description){
            // let object = this.getDescriptorOwner(description);
            // if(object){
            //     description.isCallDefinition
            //     let property = this.callee.isMemberExpression ? this.callee.property : this.callee;
            //     if(property.isIdentifier){
            //         property = property.value();
            //         if(!this.callee.isMemberExpression){
            //             property = Module.is(object) && object.id === property ? '#'+ object.id : property;
            //         }
            //         const items = object.descriptors.get(property);
            //         if(items && items.length>1){
            //             const type = description.type();
            //             return items.slice(0).sort((a,b)=>{
            //                 if(type === a.type())return -1;
            //                 if(type === b.type())return 1;
            //                 return 0;
            //             }).map( item=>item.definition(context));
            //         }
            //     }
            // }
    ...
    descriptor(){
        return this.getAttribute('CallExpression.descriptor', ()=>{
            ...
            if(!desc)return null;
            if(desc.isMethodDefinition && !desc.isAccessor || desc.isFunctionExpression || desc.isDeclaratorFunction || desc.isFunctionType){
                return desc;
            }
            ...
            const fetch = (type, exclude=null)=>{
                ...
                if(type.isLiteralObjectType){
                    let desc = type.attribute('#call#');
                    if(desc){
                        desc = desc.type();
                        if(desc && desc.isFunctionType && desc.target && desc.target.isTypeFunctionDefinition){
                            type = desc;
                        }
                    }
                }

                if(type === Fun || type.isFunctionType || type.isAnyType)return type;
    ...
    doGetDeclareFunctionType(description){
        ...
        // //return this.getCallFunctionType(description);
        // if(!description)return null;
        // if( this.callee.isSuperExpression){
        //     return this.getMatchDescriptor('constructor', this.callee.type()) || Namespace.globals.get('any');
        // }
        // if(description.isImportDefaultSpecifier || description.isImportNamespaceSpecifier || description.isImportSpecifier){
    ...
    getCallFunctionType(description){
        return this.descriptor();
        // if(!description)return null;
        // if(this.callee.isSuperExpression){
        //     return this.getMatchDescriptor('constructor', this.callee.type()) || Namespace.globals.get('any');
        // }
        // if(description.isMethodDefinition && !description.isAccessor || description.isFunctionExpression || description.isDeclaratorFunction){
        //     return description;
        // }
        // const Fun = Namespace.globals.get('Function');
        // const fetch = (type, exclude=null)=>{
        ...
        //     }
        // }
        // return fetch(description.type());
    }

    getDeclareFunctionType(type){
        if(!type)return null;
        return this.getAttribute('CallExpression.getDeclareFunctionType',()=>this.descriptor(),type);
    }
    ...
    type(){
        return this.getAttribute('CallExpression.type',()=>{
            ...
            if(final.isPredicateType){
                context.make(final.inferType());
            }else{
                context.make(final);
            }
            return final;
    ...
    presetPredicateType(declareParams, args, conditionalBlock=null){
        if(!declareParams || !declareParams.length)return;
        let type = this.getReturnType()
        if(!type || !type.isPredicateType)return;
        let pp = conditionalBlock;
        if(!conditionalBlock){
            pp = this.parentStack;
        ...
        if(pp.isIfStatement || pp.isConditionalExpression){
            ...
            if(scope && scope.allowInsertionPredicate()){
                ...
                if(existed){
                    ...
                    scope.setPredicate(lDesc, Predicate.create(assignType, rDesc, lDesc))
                }
            }
        }
    }
    ...
    parser(){
        ...
        if(description.isAnyType || description===Namespace.globals.get('Function')){
            this.parserArguments();
            return true;
        }
            ...
            const checkArguments = (index, args, declareParams, declareTypes, top=false)=>{
                ...
                for(;index<args.length; index++){
                    ...
                    if( this.isGenericsRelationValue(acceptType, declareGenerics, this.genericity) ){
                        if(acceptType.hasConstraint && !this.genericity){
                            const constraint = acceptType.inherit.type();
                            if(!constraint.hasGenericType){
                                _ctx = declareParamItem.getContext().createChild(argument);
