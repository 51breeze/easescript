...
class CallExpression extends Expression{
    ...
    definition(){
        ...
        if(description.isImportDefaultSpecifier || description.isImportNamespaceSpecifier || description.isImportSpecifier){
            if(descriptor){
                return this.definitionMergeToArray(description.definition(context), descriptor.definition(context));
            }
        }
        if(descriptor){
            return descriptor.definition(context);
        }
    }
    ...
    type(){
        return this.getAttribute('CallExpression.type',()=>{
            ...
            let final = this.getInference().apply(type);
            if(final){
    ...
    getInference(){
        ...
        return this.#inference = Inference.create(this);
    }
    ...
    parser(){
        ...
        if( length > 0 ){
            ...
            let inferContext = this.getInference();
                ...
                for(;index<args.length; index++){
                    ...
                    if( argument.isSpreadElement ){
                        ...
                        if(isRest || declareParamItem.isRestElement){
                            ...
                            if(!this.checkArgumentItemType(argument, declareParamItem, acceptType, inferContext) ){
                               // console.log("=====", acceptType.toString(), inferContext.apply(declareParamItem.type()).toString() , argument.type().toString(), acceptType.isTupleType  )
                                checkResult = false;
                            }
                        }
                    }
