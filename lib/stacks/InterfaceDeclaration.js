...
class InterfaceDeclaration extends Stack{
    ...
    async createCompleted(){
        ...
        await Promise.allSettled(this.imports.forEach(stack=>loadType(stack)));
        ...
        await Promise.allSettled(this.implements.map(stack=>loadType2(stack)));
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            await this.allSettled(this.imports,async (item)=> await item.parser() )
            await this.allSettled(this.usings,async (item)=> await item.parser() )
            ...
            await this.allSettled(this.metatypes,async (item)=> await item.parser() )
            await this.allSettled(this.annotations,async (item)=> await item.parser() )
