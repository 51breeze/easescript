...
class ImportDeclaration extends Stack{
    ...
    definition(ctx={}){
        if(this.source.isLiteral && ctx.stack === this.source){
            ...
            const file = com && !this.isResolveJsModule? com.file : this.getResolveFile();
            ...
            let kind = this.isResolveJsModule ? 'module' : 'import';
            return {
                expre:`${kind} "${file}"`,
                location,
        ...
        const head = this.alias ? '(import alias)' : '(import local)'
        return {
