...
class ImportDeclaration extends Stack{
    ...
    async descriptionAsync(){
        if( this.source.isLiteral ){
            ...
            return await this.loadTypeAsync(this.source.value()) || null;
        }
    }
    ...
    async addImport( owner, scope ){
        ...
        if( desc ){
            ...
            if(!this.namespace.has(nameId)){
                this.namespace.set(nameId, desc);
            }
    ...
    async parser(){
        return await this.callParser(async ()=>{
            if( this.source.isLiteral ){
                const resolveCompilation = await this.getResolveCompilationAsync();
                if(resolveCompilation){
                    await resolveCompilation.parserAsync();
                }
                await this.allSettled(this.specifiers, async (item)=>await item.parser() );
                if(resolveCompilation){
                    const desc = await this.descriptionAsync();
