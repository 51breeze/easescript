...
class ImportDeclaration extends Stack{
    ...
    description(){
        if(this.source.isLiteral){
            ...
            return this.importDescriptor || this.getModuleById(this.source.value()) || null;
        }
    }
    ...
    async addImport( owner, scope ){
        ...
        if( this.source.isLiteral ){
            ...
            if(result){
                ...
                ns = this.source.isMemberExpression ? Namespace.fetch(this.source.object.value(), null, true) : Namespace.top;
                if(ns){
                    property = this.source.isMemberExpression ? this.source.property.value() : this.source.value();
                    this.importDescriptor = this.getImportMatchDescriptor(ns, property)
                }
            }
        }
        ...
        if( desc ){
            ...
            if(Utils.isTypeModule(desc)){
                ...
                this.importDescriptor = desc;
                if(scope.isDefine(nameId)){
                    this.source.error(1025,nameId);
                }
                scope.define(nameId, this);
            }

            if(this.compilation.isLocalDocument()){
                if(!ns){
                    ns = this.source.isMemberExpression ? Namespace.fetch(this.source.object.value(), null, true) : Namespace.top;
                }
                if(!property){
