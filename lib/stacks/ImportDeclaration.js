    ...
    async addImport( owner, scope ){
        ...
        if( desc ){
            ...
            if(!this.compiler.options.service && this.compilation.isLocalDocument()){
                const ns = this.source.isMemberExpression ? Namespace.fetch(this.source.object.value(), null, true) : Namespace.top;
                const property = this.source.isMemberExpression ? this.source.property.value() : this.source.value();
                if(ns && ns.imports.has(property)){
                    this.addImportSpecifierDependency(ns.imports.get(property))
                }
            }
