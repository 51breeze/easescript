...
class VariableDeclarator extends Declarator {
    ...
    type(){
        ...
        if( this.parentStack.flag && type === Namespace.globals.get("any") ){
    ...
    definition(ctx){
        ...
        const expre =isProperty ? `${this.kind||''} ${token}:${type}` :`(local ${this.kind}) ${token}:${type}`;
        return {
