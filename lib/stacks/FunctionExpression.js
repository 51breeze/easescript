            ...
            if( this._returnType  ){
                ...
                if( this.parentStack.isMethodDefinition ){
                    ...
                    if(!method.isMethodSetterDefinition && !method.isConstructor){
                        ...
                        const result = this.findMethodInheritReturnType(module, key, Utils.getModifierValue(method), method.kind)
                        if(result){
            ...
            for(let item of returnItems ){
                ...
                if(argument){
                    ...
                    if(object.isThisExpression){
                        if(argument.isMemberExpression && root.isMethodDefinition && root.value() === argument.property.value() && root.module === Utils.getOriginType(object.type())){
                            continue;
                        }
                    }else if(object.isIdentifier){
                        const def = this.scope.define(object.value());
                        if(def === root){
                            continue;
                        }else if(!def && argument.isMemberExpression && root.isMethodDefinition){
                            if(root.value() === argument.property.value() && root.module){
                                continue;
                            }
                        }
                    }
                ...
                if(type.isLiteralArrayType && type.elements.length===0){
                    hasVoid = false
                    continue;
                }
