...
class MemberExpression extends Expression{
    ...
    description(){
        return this.getAttribute('MemberExpression.description', ()=>{
            ...
            if(desc && desc.isProperty && desc.computed){
                ...
                // if(scope && scope.allowPredicateType()){
                //     const predicates = scope.getPredicateType(desc);
                //     if(predicates){
                //         return predicates[1];
                //     }
                // }
    ...
    type(){
        return this.getAttribute('MemberExpression.type',()=>{
            ...
            if( description ){
                if( description.isProperty && description.computed){
                    const scope = this.scope;
                    if(scope && scope.allowPredicateType()){
                        const predicates = scope.getPredicateType(description);
                        if(predicates){
                            return predicates;
                        }
                    }
