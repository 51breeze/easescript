...
class MemberExpression extends Expression{
    ...
    getDescription(){
        ...
        if(Namespace.is(description)){
            if(this.parentStack.isMemberExpression){
                ...
                const desc = description.modules.get(property) || description.getDescriptor(property);
                if(desc && !this.compiler.options.service && this.compilation.isLocalDocument() && description.imports.has(property)){
                    this.addImportSpecifierDependency(description.imports.get(property))
                }
                return desc;
            }
        }
        else if(description){
            let object = description.isNamespaceDeclaration ? description.module : null;
            if(!object){
            ...
            if(object){
                if(this.parentStack.isMemberExpression){
                    return object.namespaces.get(property);
                }else{
                    return object.getDescriptor(property) || object.getExport(property);
                }
            }
    ...
    descriptor(){
        return this.getAttribute('MemberExpression.descriptor', ()=>{
            ...
            if(_desc){
                if(this.parentStack.isNewExpression){
