...
class MemberExpression extends Expression{
    ...
    getDescription(){
        ...
        if(description && description.isImportNamespaceSpecifier){
            if(this.parentStack.isCallExpression){
                const jsModule = description.parentStack.getResolveJSModule();
                if(jsModule){
                    let desc = this.getObjectDescriptor(description.type(), property);
                    while(desc && (desc.isImportSpecifier || desc.isImportDefaultSpecifier) ){
                        desc = desc.description();
                    }
                    if(desc && desc.isDeclaratorFunction){
                        const items = desc.module.descriptors.get(desc.value());
                        if(items && items.length>1){
                            const result = this.getMatchDescriptor(desc.value(), desc.module);
                            if(result){
                                return result;
                            }
                        }
