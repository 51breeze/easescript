...
class MemberExpression extends Expression{
    ...
    definition( context ){
        ...
        if( def && this.parentStack.isImportDeclaration && this.node.loc ){
            def.range = this.node.loc;
        }
        return def;
    }
    ...
    async descriptionAsync(){
        ...
        if( !desc ){
            const module =  this.module;
            desc = await this.loadTypeAsync( this.value() ) || null;
            if( desc && Utils.isClassType(desc) && desc !== module ){
                this.compilation.addDependency(desc, this.module);
            }
        }
        this._descAsync = [desc];
        return desc;
    }
