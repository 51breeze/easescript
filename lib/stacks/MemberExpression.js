...
class MemberExpression extends Expression{
    ...
    definition( context ){
        ...
        let def = desc.definition( context );
        if(this.isLiteralObject(object) && Utils.isMergedType(object)){
            def.expre = `(property) ${this.property.value()}: ${this.type().toString(context)}`;
        }
        return def;
    }
    ...
    description(){
        return this.getAttribute('MemberExpression.description', ()=>{
            ...
            // if( desc && !this.parentStack.isMemberExpression && !this.isTypeDefinitionStack(this.parentStack) ){
