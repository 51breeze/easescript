            ...
            if(this.parentStack.isPackageDeclaration){
                ...
                if(jsModule){
                    jsModule.getAllExportDescriptors().forEach((value, key)=>{
                        if(this.namespace.has(key) || this.namespace.descriptors.has(key)){
                            return;
                        }
                        if(value.isDeclaratorVariable ||
                            value.isDeclaratorFunction )
                        {
                            if(JSModule.is(value.module)){
                                const items = value.module.descriptors.get(key);
                                if(items){
                                    items.forEach((value)=>{
                                        if(!value.isNamespaceDeclaration){
                                            this.namespace.addDescriptor(key, value);
                                        }
                                    })
                                    return;
                                }
                            ...
                            this.namespace.addDescriptor(key, value);
                        }else if(value.isClassDeclaration || 
                            value.isDeclaratorDeclaration || 
                            value.isEnumDeclaration       || 
                            value.isInterfaceDeclaration ||
                            value.isStructTableDeclaration
                        ){
                            this.namespace.set(key, value.type());
                        }else if(value.isDeclaratorTypeAlias){
                    ...
                    if(this.compilation.isDescriptorDocument()){
                        if(this.exported){
                            let source = jsModule.id;
                            let key = this.exported.value();
                            if(!this.namespace.imports.has(key)){
                                this.namespace.imports.set(key, AutoImporter.create(source, key, '*', false, false, this));
                            }
                        }else{
