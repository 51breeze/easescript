const {isInfer} = require("../core/Inference");
        ...
        return this.getAttribute('Declarator.getRelateParamType',()=>{
            ...
            const getType = (type)=>{
                ...
                if(value && value.isGenericType){
                    value = ctx.fetch(value) || value;
                }

                if(isPattern && value){
                    let type = value.type()
                    if(type.isAliasType && type.inherit){
                        type = type.inherit.type()
                    }
                    if(isArrayPattern){
                        const index = this.parentStack.elements.indexOf(this)
                        if((type.isTupleType || type.isLiteralArrayType) && index>=0){
                            let _type =  type.elements[index]
                            if(!_type && type.isTupleType && type.prefix){
                                _type = type.elements[0]
                            }
                            return _type ? _type.type() : null;
                        }
                    }else{
                        const key = this.value()
                        if(type.isLiteralObjectType || type.isEnumType || type.isIntersectionType){
                            let _type =  type.attribute(key);
                            if(!_type && type.isLiteralObjectType){
                                _type = type.dynamicAttribute(Namespace.globals.get('string'), ctx)
                            }
                            return _type ? _type.type() : null;
                        }else if(type.isModule){
                            const desc = getDescriptor(key,(desc)=>{
                                return (desc.isPropertyDefinition || desc.isMethodGetterDefinition) && Utils.isModifierPublic(desc)
                            });
                            if(desc){
                                return desc.type()
                            }
        ...
        return this.getAttribute('getAssignType',()=>{
            ...
            if( acceptType ){
                ...
                if(index>=0){
                    ...
                    if(modifier !=="private"){
                        const result = this.findMethodInheritParamType(module, key, index, modifier, method.kind);
                        if(result){
                            return result.type();
                        }
                    }
    ...
    type(){
        ...
        if(this.parentStack.isArrayPattern && this.parentStack.parentStack.isVariableDeclarator){
            ...
            if( def ){
                const index = this.parentStack.elements.indexOf( this );
                if(index >= 0){
                    let value = Utils.extractArrayPatternItem(def.type(), index);
                    if(value){
                        return value.type()
                    }
                }
