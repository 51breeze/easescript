...
class Declarator extends Stack{
    ...
    getRelateParamDescription(stack, argument, index, propertyStack=[]){
        ...
        const fetchType=(declareParam, ctx)=>{
            ...
            }else if(argument.isArrayExpression){
                let desc = type.type();
                if( Array.isArray(propertyStack) && desc.isTupleType){
                    let itemStack = propertyStack.pop();
                    if( itemStack && itemStack.isObjectExpression ){
                        desc = desc.elements[0].type();
                        while(propertyStack.length>0){
                            const property = propertyStack.pop();
                            if(property.isProperty){ 
                                desc = ctx.getObjectDescriptorForAuxiliary(desc, property.value());
                                if(desc){
                                    desc = desc.type();
                                }else{
                                    return null;
        ...
        const getDescription = (declareParams, pos, ctx)=>{
            if( declareParams && declareParams.length > 0 ) {
                if( pos >=0 && pos < declareParams.length ){
                    return fetchType(declareParams[pos], ctx);
                }
            }
        }
        ...
        if(stack.isVariableDeclarator){
            return [
                fetchType(stack.acceptType, stack),
                stack,
                argument
            ];
        }
        ...
        if(stack.isAssignmentExpression){
            return [
                fetchType(stack.left.type(), stack),
                stack
            ];
        }
        ...
        return [
            getDescription(declareParams, stack.arguments.indexOf(argument), stack),
            stack
        ];
    }
    ...
    getRelateParamType(){
        return this.getAttribute('Declarator.getRelateParamType',()=>{
            ...
            const [description, _stack, _argument] = this.getRelateParamDescription(stack, this.parentStack, index) || [];
