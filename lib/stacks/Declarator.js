...
class Declarator extends Stack{
    ...
    getRelateParamDescription(stack, argument, index, propertyStack=[]){
        ...
        const applyClassGeneric=(type)=>{
            if(type.isClassGenericType){
                stack.getContext().assignment(type);
                return true;
            }
            if(type.isUnionType){
                type.elements.forEach(item=>applyClassGeneric(item.type()))
            }else if(type.isIntersectionType){
                applyClassGeneric(type.left.type());
                applyClassGeneric(type.right.type());
            }
        }
        ...
        const getDescription = (declareParams, pos)=>{
            if( declareParams && declareParams.length > 0 ) {
                ...
                if( pos >=0 && pos < declareParams.length ){
                    applyClassGeneric(declareParams[pos].type());
                    result = fetchType(declareParams[pos]);
                }
