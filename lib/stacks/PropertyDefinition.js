...
class PropertyDefinition extends Stack{
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            await this.allSettled(this._annotations, async item=>await item.parser() )
            await this.allSettled(this._metatypes, async item=>await item.parser() )

            await this.declarations[0].parser();
            if( this.parentStack.isUseStatement ){
                return true;
            }
