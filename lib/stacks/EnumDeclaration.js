...
class EnumDeclaration extends Stack{
    ...
    async createCompleted(){
        ...
        await Promise.all(this.imports.map( stack=>stack.addImport(this.module, this.parentStack.scope)));
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            this.inherit && await this.inherit.parser();
            await this.allSettled(this.properties, async(stack)=>{
                await stack.parser();
            });
        })
    }
