...
class DeclaratorVariable extends Stack{
    constructor(compilation,node,scope,parentNode,parentStack){
        ...
        if(!(parentStack && parentStack.isModuleDeclaration)){
    ...
    type(){
        return this.getAttribute('type',()=>{
            let type = this.declarations[0].type();
            if(type.isAliasType)return type;
            if(constants[type.id]===true)return type;
            return new InstanceofType(type, this);
        });
    }
    error(code,...args){
