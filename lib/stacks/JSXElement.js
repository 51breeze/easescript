...
class JSXElement extends Stack{
    ...
    get isComponent(){
        ...
        return !!(desc && desc.isModule && this.getGlobalTypeById('VNode') !== desc);
    }
            ...
            if( info ){
                if(info.isDirective){
                    ...
                    if( info.base ){
                        ...
                        desc = this.getModuleById(info.className) || this.getGlobalTypeById('VNode');
                    }
                    if(!desc && info.base){
                    ...
                    if( desc && desc.isModule){
                        desc = desc.type();
                        if( this.isModuleForWebComponent(desc) ){
                            this.isWebComponent = true;
                        }else if( this.isModuleForSkinComponent(desc) ){
    ...
    getElementInfo(){
        if( this.hasNamespaced ){
            ...
            if( load ){
                const base = this.isProperty ? this.parentStack.getSubClassDescription() : Namespace.create(namespace ,true);
                return {base}
            }else{
                return false;
            }
    ...
    async descriptionAsync(){
        ...
        if( info ){
            if(info.isDirective){
                ...
                if( info.base ){
                    desc = await this.openingElement.name.descriptionAsync(info.base);
                }else if(info.desc){
                    desc = info.desc;
                }else if(info.className){
                    desc = await this.loadTypeAsync(info.className) || this.getGlobalTypeById('VNode');
                }
                if(!desc && info.base){
                    const className = info.base.isNamespace ? info.base.getChain().concat( this.openingElement.name.value() ).join('.') : this.openingElement.name.value();
                    if( this.isProperty ){
    ...
    async parser(){
        ...
        return await this.callParser(async ()=>{
            ...
            const desc = this.__desc = await this.descriptionAsync();

            if( desc && desc.isModule && desc.compilation ){
                await desc.compilation.parser();
            ...
            await this.allSettled(attributes, async(item)=>{
                await item.parser();
            ...
            await this.allSettled(this.children, async (item)=> {
                await item.parser();
