...
class JSXElement extends Stack{
    ...
    addHook(){
        if(!this.hasNamespaced || this.isSlot || this.isDirective)return;
    ...
    definition( context ){
        ...
        if( stack ){
            if( this.hasNamespaced ){
                ...
                const space = this.isProperty ? Namespace.fetch(namespace, null, true) : Namespace.create(namespace ,true);
                const desc = this.openingElement.name.description( space , stack );
                return desc ? desc.definition( this.getContext() ) : null;
            }
        }

        const desc = this.description();
        if( desc && desc !== this.getGlobalTypeById('VNode') ){
