...
class ImportNamespaceSpecifier extends Declarator{
    ...
    definition(ctx){
        ...
        if( compilation ){
            ...
            if( compilation.modules.size > 0 ){
                type = compilation.mainModule || Array.from( compilation.modules.values() )[0];
            }else{
                type = this.type();
            }
            if(type){
                return {
                    ...
                    expre:`(import refers) ${this.value()}:${type.toString()}`,
           ...
           if( desc ){
                ...
                if( desc.isNamespace ){
                    ...
                    expre=`(import refers) ${this.value()}:${desc.type().toString()}`;
                }
                return {
                    comments:this.comments,
                    expre,
    ...
    toDefinition(ctx){
        ...
        if( compilation ){
            if( compilation.modules.size > 0 ){
                const module = compilation.mainModule || Array.from( compilation.modules.values() )[0];
                let def = module.definition(ctx);
                return {
                    comments:def.comments,
                    expre:`(import refers) ${this.value()}:${module.type().toString()}`,
                    location:def.location,
                    file:def.file,
                };
            }
            return {
                expre:`(import refers) ${this.value()}:${this.type().toString()}`,
