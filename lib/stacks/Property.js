...
class Property extends Stack{
    ...
    definition(ctx){
        ...
        if( this.parentStack.isObjectPattern ){
            ...
            if(desc){
                return desc.definition(ctx || this.getContext());
            }else{
    ...
    getObjectPatternDescription(){
        if(this.parentStack.isObjectPattern){
            const declare = this.parentStack.parentStack;
            if( declare && declare.isVariableDeclarator && declare.init){
                const object = declare.init.type();
                if( object ){
                    return declare.init.getObjectDescriptor(object,this.key.value());
                }
            }else if(declare && declare.isFunctionExpression && (declare.parentStack.isCallExpression||declare.parentStack.isNewExpression)){
                const call = declare.parentStack;
                let declareParams = [];
                if( declare.parentStack.isCallExpression ){
                    const description = call.getDeclareFunctionType( call.description() )
                    declareParams = call.getFunDeclareParams(description);
                }else{
                    declareParams = call.getFunDeclareParams();
                }
                const pos = declare.params.indexOf(this.parentStack);
                const index = call.arguments.indexOf(declare);
                if(declareParams[index]){
                    ...
                    if(fun.isFunctionType && fun.params[pos]){
                        ...
                        return this.getObjectDescriptor(object,this.key.value());
                    }
                }
            }
        }
    ...
    getRelateDescription(){
        ...
        if(parent && (parent.isCallExpression || parent.isNewExpression)){
            ...
            if( index>=0 ){
                const declareParams = parent.getFunDeclareParams(parent.description());
                const declare = declareParams[index];
                if( declare ){
                    let desc = declare.type();
                    while(desc && properties.length>0){
                        desc = this.getObjectDescriptorForAuxiliary(desc, properties.pop().value() );
    ...
    getContext(){
        ...
        if(acceptType){
            ...
            let init = this.init;
            if(init && init.isStack && init instanceof Stack && !init.isLiteral){
