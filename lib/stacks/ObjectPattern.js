    ...
    async parser(){ 
        return await this.callParser(async ()=>{
            ...
            if( init ){
                ...
                const p = this.parentStack && this.parentStack.parentStack;
                if( !p || !(p.isMethodDefinition || p.isFunctionExpression) ){
                    init.error(1081);
                }  
            }
            await this.allSettled(this.properties,async item=>await item.parser() );
        })
    }
    value(){
