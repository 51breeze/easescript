...
class AnnotationDeclaration extends Stack{
    constructor(compilation,node,scope,parentNode,parentStack){
        ...
        this._lowerName = String(this.node.name).toLowerCase()
                ...
                if( item.assigned ){
                    if( scope.isDefine( item.key ) ){
                        ...
                        if( desc ){
                            ...
                            scope.define(item.key, new Declarator(this.compilation, item.stack.left.node, item.stack.scope, item.stack.parentNode, item.stack.parentStack));
                        }
                    }
                }
            });
    ...
    parser(){
        ...
        switch( _name ){
            ...
            case 'embed' :
                ...
                args.forEach( item=>{
                    let file = this.compiler.resolveManager.resovleAssets(item.value, this.file);
