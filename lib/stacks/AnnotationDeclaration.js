...
class AnnotationDeclaration extends Stack{
    ...
    async createCompleted(){
        ...
        const compilations = files.map( file=>this.compilation.createChildCompilation(file, null, file, true, true) )
        ...
        this.body.map( (item,index)=>{
            if(item.isAssignmentPattern){
                ...
                const properties = item.properties.map( node=>{
                    ...
                    return {key,value,isProperty:true,stack:node, value:node.key, key:node.init.right || node.init};
                });
    ...
    async parserAsync(){
        if(super.parserAsync()===false)return false;
            ...
            case 'embed' :
                ...
                args.forEach( item=>{
                    ...
                    if( file ){
                        ...
                        if( !target.addAsset( item.resolveFile, item.value, null, path.extname(file), assign) ){
