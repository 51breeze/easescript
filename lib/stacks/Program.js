...
class Program extends Stack{
    ...
    async createCompleted(){
        ...
        await Promise.allSettled(this.body.map((stack)=>stack.createCompleted()));
        const asyncImports = [];
        ...
        await Promise.allSettled(asyncImports);
    }
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            await this.allSettled(this.metatypes, async item=>{
                await item.parser();
            })
            await this.allSettled(this.annotations, async item=>{
                await item.parser();
            })
            ...
            await this.allSettled(this.imports, async item=>{
                await item.parser();
            })

            await this.compilation.callHookAsync('program.body.parser.before');

            await this.allSettled(this.body, async item=>{
                if( !(item.isAnnotationDeclaration || item.isMetatypeDeclaration || item.isExportAllDeclaration || item.isExportDefaultDeclaration || item.isExportNamedDeclaration) ){
                    await item.parser();
                }
            })

            await this.allSettled(this.externals, async item=>{
                await item.parser();
            });

            await this.allSettled(this.exports, async item=>{
                await item.parser();
            });
