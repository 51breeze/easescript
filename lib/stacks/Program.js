...
class Program extends Stack{
    ...
    async createCompleted(){
        ...
        if(this.imports.length>0){
            await Promise.allSettled(this.imports.map(stack=>stack.addImport()));
        }
    ...
    async parserAsync(){
        ...
        await Promise.allSettled(this.body.map(async item=>{
            try{
                if( item.isAnnotationDeclaration ){
                    const name = item.name.toLowerCase();
                    if(name==='reference'){
                        return await item.parserAsync();
                    }
                }
                item.parser();
            }catch(e){
