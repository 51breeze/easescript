...
class Identifier extends Stack{
    ...
    findDescription(){
        return this.getAttribute('Idenfifier.findDescription.result',()=>{
            ...
            if(desc){
                ...
                if(desc.isDeclarator)return desc;
                if(desc.isModule && desc.id === value)return desc;
            }else{
    ...
    description(){
        ...
        const value = this.value();
        const {desc,scopeCtx,global} = this.findDescription();
        if(desc===false)return null;
        if(!desc && global){
            desc = this.getModuleById(value);
            desc = this.checkScope(desc, scopeCtx);
        }
        if(!desc && scopeCtx.removed ){
