...
class Identifier extends Stack{
    ...
    findDescription(){
        return this.getAttribute('Idenfifier.findDescription.result',()=>{
            ...
            if(desc){
                ...
                if(global){
                    desc = Namespace.globals.get(value);
                    if(!desc){
                        if(this.parentStack.isCallExpression && this.parentStack.callee === this ){
                            desc = this.parentStack.getMatchDescriptor(value, Namespace.dataset);
    ...
    description(){
        return this.getAttribute('Idenfifier.description',()=>{
            ...
            if(desc && this.scope.allowPredicateType()){
                // const predicates = this.scope.getPredicateType(desc);
                // if(predicates){
                //     return predicates[1];
                // }
            }
    ...
    type(){
        return this.getAttribute('Identifier.type',()=>{
            var type = null;
            const description = this.description();
            if(this.scope.allowPredicateType()){
                const predicates = this.scope.getPredicateType(description);
                if(predicates){
                    return predicates;
                }
            }
            ...
            if(description && description.isImportDeclaration){
