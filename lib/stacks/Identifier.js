...
class Identifier extends Stack{
    ...
    addHook(){
        if(!this.isJSXForContext()){
            const id = this.value();
            if( (this.parentStack.isCallExpression || this.parentStack.isNewExpression) && !this.scope.define(id) ){
                if( this.compilation.hasManifestResource(id, Namespace.dataset) && !this.hasModuleById(id) ){
    ...
    findDescription(){
        return this.getAttribute('Idenfifier.findDescription.result',()=>{
            ...
            if(desc && desc.isImportSpecifier && desc.parentStack && desc.parentStack.isImportDeclaration){
                ...
                if(object){
                    const _desc = this.getMatchDescriptor(value, object);
                    if(_desc)desc = _desc;
                }
            }
    ...
    async descriptionAsync(){
        ...
        if(desc===false)return null;
        if(!desc && global){
            desc = await this.loadTypeAsync(value);
        }
        if(scopeCtx && desc){
            desc = this.checkScope(desc, scopeCtx);
        }
    ...
    async parser(){
        return await this.callParser(async ()=>{
            ...
            const description = await this.descriptionAsync();
            ...
            if( !description ){
                ...
                await description.parser();
            }else if(description.isModule){
                if( description.compilation ){
                    await description.compilation.parser();
                }
            }
