                        ...
                        if(!desc){
                            if(this.parentStack.isMemberExpression){
                                ...
                                if(desc){
                                    ...
                                    if(!(desc.modules.has(property) || desc.descriptors.has(property))){
                                        desc = null;
                                    }
                                }
                            }
                            if(!desc)desc = this.getMatchDescriptor(value, Namespace.dataset);
                        ...
                        if(desc && (!this.parentStack.isMemberExpression || this.parentStack.object===this)){
                            if(!this.compiler.options.service && this.compilation.isLocalDocument()){
                                if(Namespace.dataset.imports.has(value)){
                                    this.addImportSpecifierDependency(Namespace.dataset.imports.get(value), desc)
                                    this._hasMatchAutoImporter = true;
                                }
                                // else if(Namespace.is(desc)){
                                //     const prefix = desc.imports.get(desc.id);
                                //     if(prefix && prefix.namespace){
                                //         this.addImportSpecifierDependency(prefix, this)
                                //         this._hasMatchAutoImporter = true;
                                //     }
                                // }
                            }
