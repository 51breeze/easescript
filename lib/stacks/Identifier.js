...
class Identifier extends Stack{
    ...
    addHook(){
        if(!this.isJSXForContext()){
            ...
            const maybe = pp.isCallExpression || 
                        ...
                        pp.isProperty && pp.computed && pp.node.key ===this.node ||
    ...
    resolveDefinitionContext(context){
        ...
        if(this.parentStack){
            if( this.parentStack.isStructTableDeclaration || 
                this.parentStack.isAssignmentExpression || 
                this.parentStack.isUnaryExpression || 
                this.parentStack.isTypeAssertExpression
            ){
                return null;
            }else{
        ...
        return this.getAttribute('Idenfifier.findDescription.result',()=>{
            ...
            if(_desc && _desc.isVariableDeclarator && this.hasNestedReferenceExpression(_desc.init, this)){
                ...
                if(desc.scope === this.scope){
                    desc = this.scope.parent.define(value)
                }else{
                    desc = null;
                }
            }
        ...
        return this.getAttribute('Idenfifier.description',()=>{
            ...
            if(this.parentStack.isImportDeclaration ){
                ...
                if(pp.isAnnotationExpression){
                    ...
                    if(name==='http' || name==='router'){
                        const desc = pp.getDescriptorByStack(this);
                        if(desc){
                            return desc;
                        }
                    }
