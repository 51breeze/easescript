            ...
            if(scope && scope.allowInsertionPredicate()){
                if(desc.isProperty && desc.computed && this.left.isMemberExpression){
                    ...
                    if(rType && !rType.isAnyType){
                        const existed = scope.getPredicate(desc, true);
                        if(existed){
                            existed.setAttribute(this.left.property.value(), this.right)
                        }else{
                            scope.setPredicate(desc, Predicate.attribute(this.left.property.value(), this.right));
                        }
                    }
