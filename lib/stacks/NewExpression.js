...
class NewExpression extends Expression{
    ...
    doGetConstructMethod(type, assigmentGenerics){
        let result = [];
        if( !type || type.isAnyType){
            return result;
        }
        if( type.isClassGenericType && type.isClassType ){
            return this.doGetConstructMethod(type.types[0].type(), assigmentGenerics)
        }else if(type.isClassGenericType){
            return this.doGetConstructMethod(type.inherit.type(), type.types);
    ...
    getInstanceType(){
        return this.getAttribute('NewExpression.getInstanceType',()=>{
            ...
            if(this.genericity && origin === classModule){
                ...
                const anyType = Namespace.globals.get('any')?.type();
