...
class NewExpression extends Expression{
            ...
            if( method ){
                ...
                if(result){
                    ...
                    if( _type ){
                        ...
                        classModule = ctx.apply(classModule);
                    }
    ...
    getDeclareGenerics(classModule,methodConstructor){
        if(methodConstructor && methodConstructor.isMethodDefinition && methodConstructor.genericity){
            return [methodConstructor, methodConstructor.genericity.elements];
        }
        if(classModule.isModule && classModule.isClass){
            const statcks = classModule.getStacks().filter( stack=>!!stack.genericity).sort((a,b)=>{
