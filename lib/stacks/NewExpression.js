    ...
    doGetConstructMethod(type, assigmentGenerics, exclude=null){
        ...
        if(type.isLiteralObjectType){
            ...
            if(method && method.isTypeFunctionDefinition){
                let prototype = type.attribute('prototype');
                if(prototype){
                    ...
                    if(_type.isClassGenericType){
                        if(_type.isClassType){
                            ...
                            assigmentGenerics = _type.types.map(item=>item.type());
        ...
        if( type.isClassGenericType && type.isClassType ){
            ...
            if(method && method.isDeclaratorVariable){
                return this.doGetConstructMethod(method.type(), assigmentGenerics, type)
            }
                    ...
                    if(_type && _type !== classModule){
                        if(assignType && assignType.isClassGenericType){
                            if(assignType.isClassType){
                                ...
                                assigns = assignType.types.map( item=>{
