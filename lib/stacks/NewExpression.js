...
class NewExpression extends Expression{
    ...
    definition(){
        ...
        if(methodConstructor){
            const result = methodConstructor.definition(ctx);
            if(desc.isDeclaratorVariable && result.expre.startsWith('(constructor)')){
                result.expre = `(alias ${desc.id})${result.expre.slice(13)}`
            }
            // const dataset = classModule.descriptors.get('constructor');
            // if(dataset && dataset.length>1){
            //     return dataset.slice(0).sort((a, b)=>{
            //         if(a===methodConstructor)return -1;
            //         if(b===methodConstructor)return 1;
            //         return 0;
            //     }).map( stack=>{
            //         return stack === methodConstructor ? result : stack.definition(ctx)
            //     })
            // }
            return result
        }
        if( classModule && classModule.isModule ){
            ...
            const expre = `(constructor) ${typeString}`;
            return classModule.getStacks().map( stack=>{
                return {
    ...
    doGetConstructMethod(type, assigmentGenerics, exclude=null){
        ...
        if(type.isTypeofType){
            ...
            return this.doGetConstructMethod(type.left.type(), assigmentGenerics, type) || this.doGetConstructMethod(type.right.type(), assigmentGenerics, type);
