...
class TypeGenericDefinition extends Stack {
    ...
    setRefBeUsed(){}
    type(){
        return this.getAttribute('type') || this.getModuleById(this.value()) || this.getGlobalTypeById('never');
    }

    async getTypeAsync(){
        const value = this.argument.value();
        const desc = this.isThisType ? this.module : this.scope.define(value) || await this.loadTypeAsync(value);
        if( desc ){
            type = desc.type();
            if( type ){
