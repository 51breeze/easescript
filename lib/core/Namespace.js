...
class Namespace {
    ...
    addDescriptor(name, descriptor){
        if(!descriptor || !(descriptor.isDeclaratorFunction || descriptor.isDeclaratorVariable))return false;
        let dataset = this.descriptors.get(name);
        if( !dataset ){
           dataset = [];
           this.descriptors.set(name, dataset);
        }
        if(descriptor.isDeclaratorFunction){
            if(!this.modules.has(name)){
                const type = descriptor.type();
                this.modules.set(name, type);
                if( this.fullName ){
                    globals.set(`${this.fullName}.${name}`, type);
                }else{
...
Namespace.top = Namespace.dataset
