...
class Compilation extends EventDispatcher{
    ...
    addAsset(resolve,file,content,type,assign,attrs=null,stack=null){
        if( !this.assets.has(resolve) ){
            this.assets.set(resolve, {file,resolve,content,type,assign,id:null,index:this.assets.size,attrs, stack} );
            return true;
        }
        return false;
    }
    ...
    addRequire(key,name,from,resolve,extract,stack=null){
        if( !this.requires.has(name) ){
            this.requires.set(name,{key,name,from,resolve,extract,stack,id:null,index:this.requires.size});
            return true;
        }
        return false;
    }
    ...
    clear(cache){
        ...
        this.modules.forEach((module)=>{
            ...
            module.getStacks().forEach( stack=>{
                if(stack.compilation && !cache.has(stack.compilation)){
                    stack.compilation.clear(cache);
                }
            });
        });
