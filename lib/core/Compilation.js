...
class Compilation extends EventDispatcher{
    ...
    async refresh(){
        ...
        this.compiler.printLogInfo(`refresh-relation: ${this.file} -> ${files.join('\n')} `, 'compilation')
        ...
        if(errors.length>0){
            ...
            await this.compiler.callAsyncSequence(compilations, async com=>{
                ...
                if(await com.flush()){
                    this.compiler.dispatcher('onChanged', com);
                }
    ...
    ready(){
        return new Promise( (resolve, reject)=>{
            const execute = async()=>{
                this.compiler.printLogInfo(`ready-execute-start: ${this.file}`, 'compilation')
                ...
                if(this._readying){
                    ...
                    const next = async (error)=>{
                        ...
                        while(waitings.length>0){
                            ...
                            if(error){
                                this.compiler.printLogInfo(`ready-execute-error: ${error.message} \n ${error.stack} \n by ${this.file}`, 'compilation')
                                _reject(error);
                            }else{
