...
class Compilation extends EventDispatcher{
    ...
    isLocalDocument(){
        ...
        return this.file.includes(this.compiler.workspace);
    }
    ...
    hasSameDiagnostic(node, code, range=null, kind=0){
        return this.errors.some( error=>{
            if(error.code === code && error.kind===kind ){
                if(range){
    ...
    deprecated(node,code,...args){
        ...
        const message = Diagnostic.getMessage(this.compiler.options.lang,code,args);
        const warn = new Diagnostic(this.file, message, range , 2, node,code);
        if( this.compiler.options.diagnose ){
            this.errors.push( warn );
            this.compiler.errors.push( warn );
        }
    ...
    unnecessary(node,code,...args){
        const range = this.getRangeByNode(node);
        if(this.hasSameDiagnostic(node, code, range, 3)){
            return 
        }
        const message = Diagnostic.getMessage(this.compiler.options.lang,code,args);
        const warn = new Diagnostic(this.file, message, range , 3, node,code);
        if( this.compiler.options.diagnose ){
            this.errors.push( warn );
            this.compiler.errors.push( warn );
        }
    ...
    checkCompilationDependencyOf(compilation, includeFlag=false){
        if(includeFlag){
            for(let ref of this.referenceCompilations){
                if(ref===compilation)return true;
                if(ref.checkCompilationDependencyOf(compilation, includeFlag)){
                    return true;
                }
            }
            return false;
        }else{
            return this.referenceCompilations.has(compilation);
        }
    }
        ...
        if( this.hasParsed !==true ){
            ...
            if( this.stack ){
                ...
                dependencies.sort( (a,b)=>{
                    ...
                    if(a1===0 && b1===0){
                        // if(b.checkCompilationDependencyOf(a,true)){
                        //     a1++;
                        // }
                        if(a.checkCompilationDependencyOf(b,true)){
                            b1++;
                        }
                    }
                    return a1-b1;
