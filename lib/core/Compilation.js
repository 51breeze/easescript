...
class Compilation extends EventDispatcher{
    ...
    getTypeById(id){
        ...
        }else if( this.internalDeclared.has(id) ){
            return this.getTypeValue(this.internalDeclared.get(id));
        }
    ...
    createChildCompilation(file, context, originId, notLoadDescribeFile=false, createStackFlag=true){
        const compilation = this.compiler.createCompilation(file, context, notLoadDescribeFile, false, this);
        if( compilation ){
    ...
    getModuleById(id,flag=false){
        ...
        if( !type ){
            const compilation = this.createChildCompilation( id.replace(/\./g,'/'), this.file && path.dirname(this.file), id );
            if( compilation ){
                if( flag ){
                    compilation.parser();
                }
    ...
    createModule(namespace, id, isInternal = false, isStructModule=false){
        ...
        if( !isInternal && namespace.has(id) ){
            module = namespace.get(id);
        }else if( namespace !== Namespace.dataset ) {
            name = namespace.getChain().concat(name).join('.');
        }
        if( !module ){
            module = isInternal ? this.internalDeclared.get( name ) : this.modules.get(name);
        }
        ...
        if( isInternal ){
            this.internalDeclared.set(name, module)
            return module;
        }
