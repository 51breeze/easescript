...
class Compilation extends EventDispatcher{
    ...
    checkNeedToLoadTypeById(id, context){
        ...
        id = String(id);
        const hasDot = String(id).includes('.');
        if(!hasDot && this.file){
            if(fs.existsSync(path.join(path.dirname(this.file), id+this.compiler.suffix))){
                return true;
            }
        }
        return this.hasManifestResource(id, context);
    }
    ...
    async parserAsync(source){
        if( !this.hasParsed ){
            this.hasParsed = true;
            await this.createAsync(source, null, false);
            if( this.stack ){
                this[symbolKey].parserDoneFlag = false;
                this.compiler.dispatcher('onParseStart',{compilation:this});
