...
class Compilation extends EventDispatcher{
    ...
    async callHookAsync(name){
        ...
        try{
            await Promise.allSettled(queues.map(callback=>callback()));
    ...
    async loadTypeAsync(id, context){
        let type = this.getModuleById(id, context);
        if(type===false)return null;
        if(type)return type;
        let ns = context.isNamespace ? context : this.namespace;
    ...
    async loadManifest(id, ns){
        ...
        if(manifestInfo){
            ...
            await Promise.allSettled(compilations.map( async (create)=>{
                const compilation = await create;
    ...
    async createAsync(source=null, ast=null, callCreateCompleted=true ){
        if( !this.stack && !this.stackCreating){
            ...
            if( old !== ast){
                this.stack = Tokens.create(this,ast,this.scope,null);
                await this.callHookAsync('tokens.createAsync.done');
                if( callCreateCompleted){
