...
class Compilation extends EventDispatcher{
    ...
    getReferenceName(desc, context=null){
        if(!context){
            context = this.mainModule;
        }
        if(context){
            if(Module.is(context)){
                return context.getReferenceNameByModule(desc);
            }
            if(context !== this && Compilation.is(context)){
                return context.getReferenceName(desc)
            }
        }
        if(Module.is(desc)){
            const key = desc.id;
            ...
            return desc.namespace.getChain().concat(key).join("_");
        }
        return null;
    }
    ...
    async loadTypeAsync(id, context, isImporter=false){
        ...
        if(!this.isDescriptorDocument()){
            ...
            if( idString.includes('.') ){
                file = path.resolve(this.compiler.workspace, idString.replaceAll('.', '/') + this.compiler.suffix );
            }else{
                if(isImporter){
                    file = path.resolve(this.compiler.workspace, idString + this.compiler.suffix);
                }else if(this.file){
                    file = path.resolve(path.dirname(this.file), idString + this.compiler.suffix);
    ...
    checkNeedToLoadTypeById(id, context){
        ...
        if(!this.isDescriptorDocument()){
            ...
            return fs.existsSync(path.join(this.compiler.workspace,id.replaceAll('.', '/')+this.compiler.suffix))
        }
        return false;
    }
                ...
                for(let result of matchedResult){
                    ...
                    if(props){
                        ...
                        if(name==='scope'){
                            ...
                            if( data.isDir==='true' ){
                                this.readReferenceFiles(references, data.file, context, true);
                            }else{
        ...
        if( this.hasParsed !==true ){
            ...
            if( this.stack ){
                ...
                try{
                    ...
                    if(this.changed && !this.compiler.options.service){
                        this.changed = false;
                    ...
                    this.compiler.printLogInfo(`parser-tokens-end: ${this.file} (${info.current} MB, total:${info.total} MB)`, 'compilation')
                }
            }
        }
    }
    ...
    async refresh(){
        ...
        await this.compiler.callAsyncSequence(compilations, async com=>{
            if(com.refreshing)return;
            com.refreshing=true;
            let ast = this.ast;
            com.clear();
            this.ast = ast;
            await com.parserAsync();
            this.compiler.printLogInfo(`refresh: ${com.file} errors:${com.errors.length}`, 'compilation')
            this.compiler.dispatcher('onRefreshCompleted', com);
            com.refreshing=false;
        })
    }
        ...
        return new Promise( (resolve, reject)=>{
            const execute = async()=>{
                ...
                const processor = async()=>{
                    ...
                    if(this._readying){
                        waitings.push([resolve, reject, processor])
                    }else{
                        this._readying = true;
                        let error = null;
                        const next = async ()=>{
                            ...
                            while(waitings.length>0){
                                let [_resolve, _reject, _processor] = waitings.shift();
                                if(error){
                                    _reject(error);
                                }else{
                                    _resolve(this);
                                }
                                let index = queues.indexOf(_processor);
                                if(index>=0){
                        ...
                        if(this.parseringFlag){
                            this.once('onParseDone',next)
                        }else{
                            try{
                                ...
                                if(!this.parserDoneFlag){
                                    await this.parserAsync();
                                    this.compiler.printLogInfo(`ready-finish: ${this.file}`, 'compilation')
                                }
                                resolve(this);
                            }catch(e){
                                this.compiler.printLogInfo(`ready-error: ${e.message} \n ${e.stack} \n by ${this.file}`, 'compilation')
                                reject(error=e);
                            }finally{
                                await next();
                            }
            ...
            if(this.compiler.restartuping){
                ...
                this.compiler.printLogInfo(`ready-waiting: ${this.file}`, 'compilation')
                execute()
            }
        });
    }
