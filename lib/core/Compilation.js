...
class Compilation extends EventDispatcher{
    ...
    async createChildCompilation(file, context, originId, notLoadDescribeFile=false, createStackFlag=true){
        const compilation = await this.compiler.createCompilation(file, context, notLoadDescribeFile, false, this);
        if( compilation ){
            if( !compilation.stack && !compilation.stackCreating ){
                compilation.originId = originId;
                if( !compilation.parent ){
    ...
    getModuleById(id, contextModule){
        let ns = this.namespace;
        if( contextModule && contextModule.isModule ){
            const result = contextModule.getImport(id) || contextModule.namespace.get(id);
            if( result ){
                const aliasName = contextModule.importAlias.get(result);
                if(aliasName && aliasName!==id){
                    return false;
                }
                return this.getTypeValue(result);
            }
            ns = contextModule.namespace;
        }else if(this.modules.has(id)){
            return this.getTypeValue(this.modules.get(id));
        }
    ...
    async loadTypeAsync(id, contextModule){
        let type = this.getModuleById(id, contextModule);
        ...
        await this.loadManifest(id, ns);
        if( Namespace.globals.has(id) ){
            type = Namespace.globals.get(id);
        }else{
            const compilation = await this.createChildCompilation( id.replace(/\./g,'/'), this.file && path.dirname(this.file), id );
            if( compilation ){
    ...
    async loadManifest(id, ns){
        ...
        if(this.compiler.manifester.hasRecords(id))return;
        const manifestInfo = this.compiler.manifester.getFileinfo(id, true);
        if(manifestInfo){
            const compilations = manifestInfo.files.map(file=>{
                return this.createChildCompilation(file, null, id, true, false);
            });
            await Promise.allSettled(compilations.map( async (compilation)=>{
    ...
    createStack(source, ast, callCreateCompleted=true){
        if( !this.stack ){
            const old = this.ast;
            if( !ast ){
    ...
    async createCompleted(){
        if( this.createCompletedFlag )return;
        this.createCompletedFlag = true;
        ...
        if( stack ){
            ...
            this.compiler.dispatcher('onCompilationCreateCompleted',this);
        }
    }
