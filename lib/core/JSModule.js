...
class JSModule{
    ...
    createImportDescriptors(){
        let exists = this._importDescriptors;
        if(exists)return exists;
        const properties = this._importDescriptors = new Map();
        const createAll = (object, extract=true)=>{
            let source = object.id;
            object.types.forEach((value, key)=>{
                const type = value.type();
                if(Module.is(type) && type.isClass){
                    properties.set(key, AutoImporter.create(source, key, key, extract));
                }
            })
        ...
        this.exports.forEach((value, key)=>{
            ...
            if(value.isExportDefaultDeclaration){
                ...
                if(desc.isClassDeclaration || desc.isDeclaratorDeclaration){
                    const type = desc.type();
                    if(Module.is(type) && type.isClass){
                        properties.set(type.id, AutoImporter.create(source, type.id, type.id, false, true));
                    }
                }else if(desc.isNamespaceDeclaration){
                    createAll(desc.module);
                }
            }else if(value.isExportSpecifier){
                let exported = value.exported.value();
                let local = value.local.value();
                properties.set(key, AutoImporter.create(source, exported, local, true));
                ...
                if(decl){
                    if(decl.isVariableDeclaration || decl.isDeclaratorVariable){
                        decl.declarations.forEach( decl=>{
                            ...
                            properties.set(key, AutoImporter.create(source, key, key, true));
                        });
                    }else if(decl.isNamespaceDeclaration){
                        if(decl.module){
                            createAll(decl.module);
                        }
        ...
        if(desc.isExportAssignmentDeclaration){
            ...
            if(desc){
                if(desc.isDeclaratorVariable || desc.isDeclaratorFunction){
                    ...
                    properties.set(key, AutoImporter.create(source, key, key, false));  
                    ...
                    desc.properties.forEach( (value, key)=>{
                        if(key==='default'){
                            properties.set(key, AutoImporter.create(source, key, key, false, true));
                        }else{
                            properties.set(key, AutoImporter.create(source, key, key, true)); 
                        }
                    })
                }else if(desc.isNamespaceDeclaration && desc.module && nsModule !== desc.module && desc.module !== this){
                    createAll(desc.module);
                }
            }
            ...
            if(desc.source){
                ...
                if(jsModule){
                    if(desc.exported){
                        ...
                        properties.set(key, AutoImporter.create(source, key, '*'));
                    }else{
                        jsModule.createImportDescriptors().forEach( (value, key)=>{
                            properties.set(key, value);
                        })
                    }
                }
