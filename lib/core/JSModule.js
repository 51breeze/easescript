        ...
        this.exports.forEach((value, key)=>{
            ...
            if(value.isExportDefaultDeclaration){
                ...
                if(desc.isClassDeclaration || desc.isDeclaratorDeclaration){
                    const type = desc.type();
                    if(Module.is(type) && type.isClass){
                        properties.set(type.id, createItem(source, type.id, type.id, false, value, desc, this, true));
                    }
                }else if(desc.isNamespaceDeclaration){
                    properties.set(desc.module.id, createItem(source, desc.module.id, desc.module.id, false, value, desc, this, true));
                }
            }else if(value.isExportSpecifier){
                let exported = value.exported.value();
                let local = value.local.value();
                properties.set(key, createItem(source, exported, local, true, value, value.description(), this));
            ...
            if(exported){
                ...
                if(desc){
                    if(desc.isModuleDeclaration){
                        ...
                        return desc.module.getDescriptor(desc.value(), filter, {isNew,isCall}, result)
                    }
                }
                return desc;
            }else{
                ...
                if(desc.isExportAllDeclaration){
                    if(desc.source){
                        const jsModule = desc.getResolveJSModule();
                        if(jsModule){
                            return jsModule.getDescriptor(name, filter, {isNew,isCall}, result)
                        }else{
                    ...
                    if(desc.expression.isIdentifier){
                        ...
                        if(nsModule){
                            return nsModule.getDescriptor(name, filter, {isNew,isCall}, result)
                        }else{
