...
class JSModule{
    ...
    static getDescriptor(id, filter=null){
        if(!id || id==='global'){
            return null;
        }
        const items = id.split(".");
        ...
        if(filter){
            return jsModule.getDescriptor(name, filter)
        }
        let result = base.descriptors.get(name);
    ...
    static getGlobalNamespaceModule(){
        const glob = JSModule.getNamespace();
        return glob.modules.get('global');
    }
    ...
    static getModuleFromNamespace(id){
        ...
        let module = base.modules.get('global');
        while(key=items.shift()){
            if( base.children.has(key) ){
                base = base.children.get(key);
                module = base.modules.get(key);
    ...
    getExport(name){
        ...
        if(desc.isExportAssignmentDeclaration){
            ...
            if(isRefs){
                ...
                if(nsModule){
                    let desc = nsModule.getDescriptor(name) || nsModule.getExport(name);
                    if(desc){
                        return desc;
                    }
                }
            ...
            if(desc){
                if(desc.isObjectExpression){
                    ...
                    return desc.getExport(name)
                }else if(!isRefs){
                    return this.getExportFromType(desc.type(), name);
                }
            }
            ...
            if(desc.source){
                ...
                if(jsModule){
                    return jsModule.getExport(name)
                }else{
                    return desc.getAllExportDescriptors().get(name);
                }
            }else{
                const jsModule = desc.module;
                if(jsModule){
                    return jsModule.getDescriptor(name);
                }
            }
        }
        return null;
    ...
    getExportObjectType(){
        ...
        const object = new LiteralObjectType(Namespace.globals.get('object'), null, new Map(this.exports));
