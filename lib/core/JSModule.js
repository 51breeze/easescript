...
class JSModule{
    ...
    getExport(name, flag=false){
        ...
        if(starDesc)items.push(starDesc);
    ...
    getAllExportDescriptors(){
        ...
        this.exports.forEach((value, key)=>{
            if(value.isExportAllDeclaration || value.isExportAssignmentDeclaration){
                exportAllDecls.push(value);
                return;
            }
    ...
    getModuleDefaultDesriptor( property=null ){
        ...
        for(let [key, exported] of this.exports){
            ...
            let result = null;
            if(exported.isExportAssignmentDeclaration){
                result = get(exported.description(), property && exported.expression.isIdentifier ? exported.expression.value() : null)
            }else if(exported.isExportAllDeclaration){
                if(exported.source){
                    const jsModule = exported.getResolveJSModule();
                    if(jsModule){
    ...
    createImportDescriptors(source){
        ...
        this.exports.forEach((value, key)=>{
            if(value.isExportDefaultDeclaration){
            ...
            else if(value.isExportAssignmentDeclaration){
                const isRefs = value.expression.isIdentifier;
                let nsModule = null;
                if(isRefs){
                    nsModule = this.namespaces.get(value.expression.value());
                    if(nsModule){
                ...
                if(value.source){
                    const jsModule = value.getResolveJSModule();
                    if(jsModule){
                        if(value.exported){
                            let key = value.exported.value();
                            properties.set(key, createItem(source, key, '*', true, value, jsModule, jsModule));
                        }else{
                        ...
                        value.getAllExportDescriptors().forEach((value, key)=>{
                            properties.set(key, value);
                        })
                    }
                }else if(JSModule.is(value.module)){
                    createAll(value.module, source);
                }
            }
        });
