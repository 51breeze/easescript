...
class Compiler extends EventDispatcher{
    ...
    getFileAbsolute(file, context, flagSuffix = true, checkNodeModules=true, onlyWorkspace=false){
        ...
        if( path.isAbsolute( file )){
            ...
            if(context){
                ...
                while( root && !(file = resolve(root,name) ) && section.pop() ){
                    root = section.join("/");
                    if( onlyWorkspace ){
                        if( !root.includes(this.workspace) ){
                            return null;
                        }
                    }
                }
    ...
    async loadDescriptorFiles( dirname ){
        const descFile = this.resolveAppointFile('index.d.es',dirname);
    ...
    async loadTypes(types, pluginScope){
        ...
        await Promise.allSettled(compilations.map(compilation=>compilation.createAsync()));
