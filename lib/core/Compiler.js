...
class Compiler extends EventDispatcher{
    ...
    defaultOptions(){
        return {
            ...
            annotations:[
                ...
                'DOMAttribute','Injector','Reactive','Hook','URL','Http','Version','Removed'
            ],
            jsx:{
    ...
    async createCompilation(file, context=null, flag=false, isRoot=false, parentCompilation=null){
        ...
        if( file ){
            ...
            if( fs.existsSync(file) ){
                ...
                if( resourcePath.includes( this.lowerWorkspace ) ){
    ...
    resolveTypingsFromPackage(jsonFile, dataset=null, typings=null){
        ...
        if( !jsonFile || DescriptorLoadedFolderCache[jsonFile]){
            return null;
        }
        ...
        folders.forEach((folder)=>{
            ...
            if(file && !DescriptorLoadedFolderCache[file]){
                ...
                if(scanDependencyFlag){
                    ...
                    deps.forEach( dep=>{
                        this.resolveTypingsFromPackage(path.join(dep, 'package.json'), dataset);
                    });
                }
            }
        });
        ...
        return new Promise( async(resolve,reject)=>{
            ...
            TaskCache.queues.push(async()=>{
                ...
                if(compilation){
                    ...
                    reject(new Error(`The "${file}" is not resolve.`));
                }
                if(TaskCache.queues.length>0){
                    const task = TaskCache.queues.shift();
                    await task();
                }else{
