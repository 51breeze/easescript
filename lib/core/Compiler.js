...
class Compiler extends EventDispatcher{
    ...
    async createCompilation(file, context, flag, isRoot, parentCompilation=null){
    ...
    async loadDescriptorFiles( dirname ){
        const descFile = this.resolveIndexDescriptorFile('index.d.es',dirname);
        ...
        const file = this.resolveAppointFile('package.json', dirname);
        if( file ){
    ...
    resolveAppointFile(filename, folder, entry=null, prevs=[], ctx=null){
        ...
        if( fs.existsSync(file) ){
            ...
            const result = this.resolveAppointFile(filename, path.dirname(folder), entry, prevs, ctx);
    ...
    async scanTypings(folders, scanDependencyFlag = true, dataset=null){
        ...
        folders.forEach((folder)=>{
            const file = this.resolveAppointFile('package.json', folder);
    ...
    async initialize(){
            ...
            options.types.forEach( file=>{
                if( fs.existsSync(file) && fs.statSync(file).isFile() ){
                    if( suffix.test(file) ){
                        ...
                        const json = this.resolveAppointFile('package.json',folder);
                        if( json ){
        ...
        if( dataset && dataset.size > 0 ){
            await Promise.allSettled(Array.from(dataset.values()).map( item=>this.loadTypes(item.files, item.esconfig)));
        }
    ...
    async loadTypes(types, pluginScope){
        ...
        await Promise.allSettled(types.map(async file=>{
