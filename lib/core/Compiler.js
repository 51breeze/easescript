...
class Compiler extends EventDispatcher{
    ...
    parseConfigFile(options={}){
        ...
        const file = options.configFileExtensions.map(ext=>path.join(options.cwd, name+ext)).find( file=>fs.existsSync(file));
        ...
        if( !fs.existsSync(options.workspace) ){
            ...
            if(!path.isAbsolute(options.workspace)){
                ...
                for(let i=0; i<tries.length;i++){
                    if(!tries[i])continue;
                    resolvePath = this.getWorkspaceFolder(cwd, tries[i]);
                    if(resolvePath){
                        console.warn( `Resolved workspace path '${resolvePath}' successfully.`);
                        break;
                    }else{
                        console.warn( `Resolved workspace path '${path.join(cwd,tries[i])}' failed.`);
                    }
                }
                if(!resolvePath){
        ...
        compilation.namespaceSets.forEach( ns=>{
            ns.modules.forEach( (item,name)=>{
                ...
                if(isLocal && !inheritScopes.includes(compilation.pluginScopes.scope) ){
                    const compiFiles =compilations.map(compi=>compi.file);
