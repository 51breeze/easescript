...
function makeContext(stack){
    ...
    }else if(stack.isVariableDeclarator || stack.isParamDeclarator){
        let type = stack.type();
        origin = Utils.getOriginType(type);
        if(!Utils.isModule(origin)){
            origin = null;
...
function create(stack, parent=null, type=null){
    currentStack = stack;
    let unknown = null;
    let group = null;
    let origin=null, records=null, post=null;
    if(type && Utils.isType(type)){
        records =  new Map()
        origin = Utils.getOriginType(type);
        if(!Utils.isModule(origin)){
            origin = null;
        }
        extractContextGenerics(records, type)
    }else{
        let result = makeContext(stack);
