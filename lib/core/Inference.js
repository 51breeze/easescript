...
function findMatchTypeByGenerics(declareGenerics, declareParams, args, assigmentGenerics, type){
    let len = args.length;
    let generics = extractGenericsFromType(type).filter(g=>declareGenerics.some(decl=>decl.type().getUniKey()===g.getUniKey()));
    ...
    for(let i=0;i<len;i++){
        ...
        if(declParamType.hasGenericType){
...
function callReturnType(stack, type){
    ...
    let results = findMatchTypeByGenerics(declareGenerics, declareParams, args, assigmentGenerics, type);
