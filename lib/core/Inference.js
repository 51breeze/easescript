...
function extractGenericsFromType(type, records=new WeakSet()){
    if(!type || records.has(type))return [];
    if(type.isGenericType)return [type];
    records.add(type);
    if(type.isAliasType){
        return extractGenericsFromType(type.inherit.type(), records);
    }
    let elements = null;
...
function getTypeDeclareGenerics(type, origin=null){
    if(type.isAliasType && type.target && type.target.isDeclaratorTypeAlias && type.target.genericity){
        ...
        const segs = type.target.getDeclareGenerics();
        return Array.isArray(segs[1]) ? segs[1].map(item=>item.type()) : [];
...
function extractContextGenerics(records, context, origin=null, relateGenerics=[], excludeTuple=false){
    ...
    if(context.isAliasType){
        ...
        if(!excludeTuple){
            ...
            if(assigmentType && assigmentType.type().isTypeofType){
                assigmentType = assigmentType.type().origin.type()
            }
            classDeclareGenerics.forEach(decl=>{
                let index = findGenericIndex(relateGenerics, decl);
