...
class Stack extends EventDispatcher{
    ...
    hasModuleById(id){
        return this.compilation.hasModuleById(id, this.module||this.namespace);
    }

    async loadTypeAsync(id){
        return await this.compilation.loadTypeAsync(id, this.module||this.namespace);
    }
    ...
    async parserAsync(){
        if( this.__asyncParsered )return false;
        this.__asyncParsered = true;
    }
    ...
    async allSettled(items, asyncMethod, fetchResult=false){
        ...
        try{
            //await this.compiler.callSequential(items.map((item,index)=>asyncMethod(item,index,items)))

            const fns = items.map( (item,index)=>async ()=>await asyncMethod(item,index,items) )
            return await this.compiler.callSequential(fns)
