...
class Stack extends EventDispatcher{
                            ...
                            if(code===64){
                                ...
                                if(value){
                                    ...
                                    if(value){
                                        ...
                                        if(value && (prop ==='params' || prop ==='param') && typeof value ==='string'){
                                            let [_name, _comment] = value.split(/\s+/, 2);
                                            data.params.push({
                                                label:_name,
                                                comment:_comment
                                            });
    ...
    formatHover(def, items){
        ...
        items.forEach(item=>{
            ...
            let prefix = item ===def || text.charCodeAt(0) ===40 ? '' : '(alias) ';
            if(!cache[text]){
                message += prefix+text+ '\n';
                cache[text] = true;
            }
    ...
    getTypeDisplayName(type, ctx, options={}){
        ...
        if(Utils.isContext(ctx)){
            if(type.isConditionalExpressionType || type.isInferGenericType){
                type = type.getResult(ctx)
            }
        }
        ...
        if(object.isClassGenericType){
            if(object.isClassType){
                ...
                if( wrap.target && wrap.target.isDeclaratorTypeAlias && wrap.target.genericity ){
                    ...
                    const ctx = Inference.getInferenceFromType(object);
    ...
    getObjectDescriptorForAuxiliary(object, property, isStatic=false, prevObject=null, context=null){
        ...
        if( object.isIntersectionType ){
            return this.getObjectDescriptorForAuxiliary(object.left.type(), property, isStatic, object, ctx) || 
                    this.getObjectDescriptorForAuxiliary(object.right.type(), property, isStatic, object, ctx);
        }
        let dynamicAttribute = false;
        let result = null;
        if( object.isUnionType ){
            ...
            for(let i=0;i<elems.length;i++){
                const item = elems[i];
                const result = this.getObjectDescriptorForAuxiliary(item.type(), property, isStatic, object, ctx);
                if( result ){
                    return result;
                }
            }
    ...
    isTypeDefinitionStack(stack){
        ...
        return stack.isTypeDefinition || 
                ...
                stack.isTypeConditionDefinition || 
                stack.isTypeInferDefinition || 
    ...
    parseConditionState(stack){
        ...
        this.getConditions(stack).forEach(stack=>{
            ...
            if(desc && (!stack.parentStack.isLogicalExpression || stack.parentStack.isAndOperator)){
    ...
    findMethodInheritParamType(module, key, index, modifier, kind){
        if(!(module.extends.length>0 || module.implements.length>0))return null;
        const imps = [...module.extends, ...module.implements].map(imp=>imp.type());
        for(const impModule of imps){
            if(modifier==="protected" && Utils.isInterface(impModule))continue;
    ...
    findMethodInheritReturnType(module, key, modifier, kind, isStatic=false){
        if(!(module.extends.length>0 || module.implements.length>0))return null;
        const imps = [...module.extends, ...module.implements].map(imp=>imp.type());
        for(const impModule of imps){
            if(modifier==="protected" && Utils.isInterface(impModule))continue;
            ...
            const result = this.findMethodInheritReturnType(impModule, key, modifier);
            if(result)return result;
        }
        return null;
    }

    getIteratorResultType(type, ctx){
    ...
    hasNestDescription(stack){
        ...
        if(desc.isFunctionExpression){
            const parent = this.getParentStack(parent=>parent.isFunctionExpression && (parent===desc || parent.isMethodDefinition || parent.isVariableDeclarator));
            return parent===desc;
        }
        return false;
    }
