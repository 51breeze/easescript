...
class Stack extends EventDispatcher{
    ...
    parseConditionState(stack){
        ...
        const getConditions=(stack)=>{
            if(stack.isLogicalExpression){
                return [getConditions(stack.left), getConditions(stack.right)].flat();
            }else if(stack.isParenthesizedExpression){
                return getConditions(stack.expression);
            }
            return [stack];
        }
        getConditions(stack).forEach(stack=>{
            ...
            const desc =stack.isMemberExpression ? stack.object.description() : stack.description();
            if(desc){
