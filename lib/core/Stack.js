...
class Stack extends EventDispatcher{
        ...
        return this.getAttribute('getLocalReferenceType:'+id,()=>{
            ...
            if(!type){
                if(JSModule.is(this.module)){
                    type = this.module.getType(id) || this.getModuleById(id);
                }else{
    ...
    hasLocalReferenceType(id){
        return this.getAttribute('hasLocalReferenceType:'+id,()=>{
            ...
            if(!type){
                if(JSModule.is(this.module)){
                    type = this.module.getType(id) || this.module.getDescriptor(id) || this.getModuleById(id);
                }else{
    ...
    getMatchDescriptor(property, classModule, isStatic=false, onlyAccessProperty=false){
        if(!property || !classModule || !(classModule.isModule || classModule.isNamespace || classModule.isUnionType && property==='#match-union-type'))return null;
    ...
    getObjectDescriptor(object, property, isStatic=false, prevObject=null, context=null){
        ...
        if( !result ){
            ...
            if(literalType){
                result = this.getObjectDynamicDescriptor( dynamicAttribute ? object : origin, literalType, ctx);
            }
        }
    ...
    getObjectDescriptorForAuxiliary(object, property, isStatic=false, prevObject=null, context=null){
        ...
        if( !result ){
            result = this.getObjectDynamicDescriptor( dynamicAttribute ? object : origin, object.isLiteralArrayType || object.isTupleType ? 'number' : 'string', ctx)
        }
        return result;
    }
    ...
    getObjectDynamicDescriptor(object, propertyType, ctx=null){
        if( this.isLiteralObject(object) || Utils.isTypeModule(object) ){
            const tof = typeof propertyType;
            const type = tof === 'string' || tof === 'number' ? Namespace.globals.get(propertyType) : propertyType;
            ...
            if(type && type.isType){
                return object.dynamicAttribute(type, ctx);
            }
        }
        return null;
    }
