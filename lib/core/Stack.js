    ...
    checkMatchType(argumentStack, declareParam, acceptType, context){
        ...
        if( declareParam && declareParam.isObjectPattern ){
            ...
            if( acceptType.isGenericType ){
                ...
                const aParams = acceptType.params.filter(p=>{
                    return !(p.isAssignmentPattern || p.question || p.isRestElement)
                })
                const bParams = argumentType.params.filter(p=>{
                    return !(p.isAssignmentPattern || p.question || p.isRestElement)
                });
                if(aParams.length>0 && aParams[aParams.length-1].isRestElement){
                    return true;
                }
