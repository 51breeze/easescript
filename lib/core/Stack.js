...
class Stack extends EventDispatcher{
    ...
    getMatchDescriptor(property, classModule, isStatic=false, onlyAccessProperty=false){
        ...
        const calcScore=(result, params, generics, incompleteFlag)=>{
        ...
        const choose = (prev, desc, params, generics)=>{
            ...
            // let pParamLen = prev[1].length;
            // let pGenericsLen = prev[2].length;
            // let pMatchResult = prev[3];
            // let pIncompleteFlag = prev[4];
            // let cParamLen = params.length;
            // let cGenericsLen = generics.length;
            // let cIncompleteFlag = incompleteFlag;
            // let cMatchResult = matchResult;

            let pScore = calcScore(prev[3], prev[1], prev[2], prev[4])
            let cScore = calcScore(matchResult, params, generics, incompleteFlag)
            ...
            //     }
            // }
            // if( cParamLen > pParamLen || cGenericsLen > pGenericsLen ){
            //     return update(desc, params, generics);
            // }
            // return prev;
        }
        const filter = (desc, prev, index, descriptors, extendsContext)=>{
            ...
            if( isCall ){
                ...
                if( desc.isMethodDefinition && !desc.isMethodGetterDefinition || desc.isDeclaratorFunction || isNew && desc.isNewDefinition || !isNew && desc.isCallDefinition){
                    ...
                    if(isNew){
                        if(desc.isConstructor && !desc.isNewDefinition && desc.module===classModule){
