...
class Stack extends EventDispatcher{
    ...
    getCache(name){
        ...
        if(!data){
            if(Stack.#lastCacheId){
               delete this[keySymbol][Stack.#lastCacheId];
            }
            Stack.#lastCacheId = id;
    ...
    hasAttribute(name){
        const data = this.getCache('attributes');
        ...
        this.getConditions(stack).forEach(stack=>{
            ...
            if(desc){
                ...
                if(old && old.value !== value){
                    ...
                    scope.setValidateState(desc, this, value, stack, pp)
