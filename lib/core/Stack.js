...
class Stack extends EventDispatcher{
    ...
    setPredicateType(desc, type, scope){
        if(scope){
            if(scope.type('block')){
                scope.setPredicateType(desc, type)
            }
            return;
        }
        const dataset = this[keySymbol][predicateSymbol] || (this[keySymbol][predicateSymbol] = new Map());
        dataset.set(desc, type);
    }

    getPredicateType(desc, scope){
        const dataset = this[keySymbol][predicateSymbol] || (this[keySymbol][predicateSymbol] = new Map());
        let typemap = dataset.get(context);
        if( desc ){
    ...
    getMatchDescriptor(property, classModule, isStatic=false, onlyAccessProperty=false){
        ...
        const filter = (desc, prev, index, descriptors, extendsContext)=>{
            ...
            if( isCall ){
                ...
                if( desc.isMethodDefinition && !desc.isMethodGetterDefinition || desc.isDeclaratorFunction || isNew && desc.isNewDefinition || !isNew && desc.isCallDefinition){
                    generics = desc.genericity ? desc.genericity.elements : emptyOnlyreadArray;
                    params = desc.params || emptyOnlyreadArray;
                    if(isNew){
                        if(!(desc.isConstructor || desc.isNewDefinition) )return false; 
    ...
    getObjectDescriptor(object, property, isStatic=false, prevObject=null, context=null){
        ...
        if( object.isUnionType ){
            ...
            for(let i=0;i<elems.length;i++){
                ...
                if(!objectType || objectType.isAnyType || objectType.isNullableType){
            ...
            if(!isFullMatch){
                this.warn(1188, property)
            }

            if(hasNull && this.isMemberExpression && !this.optional){
                let state = null;
                let scope = this.scope;
                if(scope && (scope.isBlockScope || scope.isFunctionScope) ){
                ...
                if(!state || !state.value){
                    ...
                    if(pp && pp.isLogicalExpression && pp.operator.charCodeAt(0) === 38){
                        let refs = null;
                        ...
                        const item = items.find( item=>{
                            ...
                            return refs.isMemberExpression ? refs.getFirstMemberStack() === this.getFirstMemberStack() : false;
                        });
                        if(item && refs){
                            refs = refs.isMemberExpression ? refs.object : refs;
                            const refsDesc = refs.description();
                            const leftItem = items.find( item=>item.isIdentifier || item.isMemberExpression ? item.description() === refsDesc : false);
                            if(leftItem && leftItem.parentStack.isLogicalExpression){
                                if(leftItem.parentStack === item.parentStack){
                                    hasCheck = true;
                                }else{
                                    hasCheck = leftItem.parentStack.operator.charCodeAt(0) === 38;
                                }
                            }
                        }
                    }
    ...
    setRefBeUsed( description ){
        ...
        if( desc && desc instanceof Stack && desc !== this ){
            ...
            if(classStack && classStack !== target ){
                classStack.addUseRef( target );
            }
    ...
    parseConditionState(stack){
        if(!stack)return;
        const scope = stack.scope;
        if(!(scope && scope.isBlockScope))return;
        this.getConditions(stack).forEach(stack=>{
            if(!(stack.isIdentifier || stack.isMemberExpression || stack.isUnaryExpression))return;
            stack.parser();
            let value = true;
            if(stack.isUnaryExpression){
                if( stack.operator.charCodeAt(0) === 33){
