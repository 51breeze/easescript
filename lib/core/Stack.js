...
class Stack extends EventDispatcher{
    ...
    getTypeById(id){
        return this.compilation.getModuleById(id, this.module);
    }
    ...
    async loadTypeAsync(id){
        return await this.compilation.loadTypeAsync(id, this.module);
    }
    ...
    async allSettled(items, asyncMethod, fetchResult=false){
        ...
        const results = await Promise.allSettled(items.map(item=>asyncMethod(item)));
        if(fetchResult){
