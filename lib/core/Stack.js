...
class Stack extends EventDispatcher{
    ...
    getTypeById(id){
        if( this.module ){
            let type = this.module.getImport(id) || this.module.namespace.get( id );
            if( type ){
    ...
    getModuleById(id, flag = false){
        ...
        }else if( this.compilation.internalDeclared.has(id) ){
            return this.compilation.internalDeclared.get(id)
        }else if(this.namespace){
            const fullName = this.namespace.getChain().concat(id).join('.')
            if( this.compilation.internalDeclared.has(fullName) ){
                return this.compilation.internalDeclared.get(fullName);
            }
        }
        ...
        if( declareParam.isObjectPattern ){
            ...
            if( argumentType.isLiteralObjectType || argumentType.isInstanceofType || argumentType.isEnumType || (argumentType.isEnum && argumentType.isModule) ){
                declareParam.properties.forEach( property=>{
                    let propertyStack = (argumentType.isEnum && argumentType.isModule) ? argumentType.getMember(property.key.value(),'get') : argumentType.attribute( property.key.value(), 'get');
            ...
            }else if( !argumentType.isAnyType ){
                const objectType = this.getGlobalTypeById('object');
                if( !objectType.is(argumentType,context) ){
                    checkResult = false;
                    (whenErrorStack || argumentStack).error(1002, this.getTypeDisplayName( argumentType, context ), this.getTypeDisplayName(objectType,context) );
                }
            }
    ...
    isModuleForWebComponent(module){
        ...
        if( module.metadataFlags > 0 ){
            if( (Constant.MODULE_YES_WEB_COMPONENT & module.metadataFlags) === Constant.MODULE_YES_WEB_COMPONENT ){
                return true;
            }else if( (Constant.MODULE_NO_WEB_COMPONENT & module.metadataFlags) === Constant.MODULE_NO_WEB_COMPONENT ){
                return false;
            }
        }
        let result = false;
        let webComponent = module;
        while( webComponent ){
            const stack = this.compilation.getStackByModule( webComponent );
            if( stack && stack.annotations ){
                if( stack.annotations.some( item=>item.name.toLowerCase() === 'webcomponent' ) ){
                    result = true;
                    break;
                }
            }
            webComponent = webComponent.inherit;
        }
        module.metadataFlags |= result ? Constant.MODULE_YES_WEB_COMPONENT : Constant.MODULE_NO_WEB_COMPONENT;
        return result;
    }
    ...
    isModuleForSkinComponent(module){
        ...
        if( module.metadataFlags > 0 ){
            if( (Constant.MODULE_YES_SKIN_COMPONENT & module.metadataFlags) === Constant.MODULE_YES_SKIN_COMPONENT ){
                return true;
            }else if( (Constant.MODULE_NO_SKIN_COMPONENT & module.metadataFlags) === Constant.MODULE_NO_SKIN_COMPONENT ){
                return false;
            }
        }
        let result = false;
        ...
        while( webComponent ){
            const stack = this.compilation.getStackByModule( webComponent );
            if( stack && stack.annotations ){
                if( stack.annotations.some( item=>item.name.toLowerCase() === 'skinclass' ) ){
                    result = true;
                    break;
                }
            }
        ...
        module.metadataFlags |= result ? Constant.MODULE_YES_SKIN_COMPONENT : Constant.MODULE_NO_SKIN_COMPONENT;
        return false;
    }

    findAnnotation(stack, callback){

        if( stack && stack.isModule && stack instanceof Module){
            stack = this.compilation.getStackByModule(stack);
        }
        ...
        let result = each(stack.annotations, (annotation)=>{
            const result = callback(annotation, stack);
            if( result ){
                return [result, stack];
            }
        });
        ...
        if( stack.isClassDeclaration || stack.isDeclaratorDeclaration || stack.isInterfaceDeclaration || stack.isEnumDeclaration ){
            ...
            return each( impls, (module)=>{
                const result = this.findAnnotation(this.compilation.getStackByModule(module), callback);
                if( result ){
                    return result;
                }
            });
            ...
            const impls = module.extends.concat( module.implements || [] );
            const kind = stack.isMethodSetterDefinition ? 'set' : 'get';
            const token = stack.toString();
            const getMermber = (module)=>{
                const members = module.members;
                if( members && Object.prototype.hasOwnProperty.call(members,name) ){
            ...
            return each( impls, (module)=>{
                const stack = getMermber(module);
                if( stack && token === stack.toString() ){
                    const result = this.findAnnotation(stack, callback);
                    if( result ){
                        return result;
                    }
                }
    ...
    interceptAnnotation( stack ){
        ...
        if( aName ==="hostcomponent" && this.isJSXScript ){
            ...
            // if( this.compilation.import ==='reference' ){
            //     return null;
            // }
            const describePattern = this.compiler.options.describePattern;
            stack.getArguments().forEach( item=>{
                const files = [];
                const deep = (file,context)=>{
                ...
                deep( item.value , this.file );
                const compilations = new Set();
                
                files.forEach( file=>{
                    const compilation = this.compilation.createChildCompilation(file, null, null, true, false);
                    if( !compilation ){
                        item.stack.error(1132, file);
                    }else if( !compilations.stack && !compilation.stackCreating ){
