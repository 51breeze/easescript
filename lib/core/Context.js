...
class Context{
    ...
    assignment(type,callback){
        ...
        if( type.isClassGenericType ){
            [stack,declareGenerics] = type.target.getDeclareGenerics();
        }else if(originType && originType.isModule){
            [stack,declareGenerics] = originType.getModuleDeclareGenerics(true)
        }
    ...
    apply(type, context={}){
        if(type && type.hasGenericType){
            ...
            if(type.isGenericType){
                return this.fetch(type) || (this._applyContext.mismatch=true,UKType);
            }
