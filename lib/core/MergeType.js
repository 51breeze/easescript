...
class MergeType{
    ...
    createGroup(stack){
        ...
        if( type.isLiteralObjectType ){
            ...
            dataset.isTupleUnion = MergeType.isTupleUnion( type );
    ...
    add( stack , toLiteralValue=false){
        ...
        if( type ){
            if( type.isLiteralValueType ){
                ...
                this.types.set(type.toString(null, {toLiteralValue:true}),  type);
            }
            else if( type.isLiteralArrayType || type.isTupleType ){
                const originType = Utils.getOriginType(type);
    ...
    type(){
        ...
        if( this.originType === origArrayType ){
            ...
            return new TupleType(origArrayType, elements, this.target, false, this.isTupleUnion );
        ...
        const items = Array.from( this.types.values() ).map(item=>item.type() )
        ...
        if( !(items.length>0) || items.some( type=>{
            type = type.type();
            return type.isAnyType && !type.isUnknownType;
        })){
            return Namespace.globals.get('any');
        }

        if( items.length > 1 ){
            const temp = items.filter( item=>!!item.type().isLiteralObjectType);
            if(temp.length>1){
                temp.forEach( item=>{
                    const type = item.type();
        ...
        if( items.length > 1  ){
            ...
            let isNumber = items.every( (item)=>{
                const name = item.toString();
                if( name==="number" )typeNumber = item;
                if( name==="boolean" )typeBoolean = item;
