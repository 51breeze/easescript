...
class Module extends Type{
    ...
    addDescriptor(name, descriptor){
        if(descriptor.isRemoved){
            ...
            this[symbolKey].removed[name] = true;
            return true;
        }

        if(this[symbolKey].removed[name] === true){
            return true;
        }
    ...
    clear( compilation ){
        ...
        this[symbolKey].stacks.length = 0;
    ...
    is( type, context ){
        ...
        if( this.isInterface && type.isLiteralObjectType ){
    ...
    getModuleGenerics(){
        const stackModule = this.moduleStack;
    ...
    toString( context, options={}){
        ...
        if( context && context.stack){
            if( context.stack === stackModule.id ){
        ...
        if( stackModule && stackModule.genericity ){
            ...
            const elements = stackModule.genericity.elements.map( item=>{
