...
class Module extends Type{
    ...
    isRemoved(name){
        ...
        if(this.extends.some( item=>item.isRemoved(name))){
            return true;
        }
        if(this.implements.some( item=>item.isRemoved(name))){
            return true;
        }
        return false;
    }
    ...
    isDeprecated(name){
        ...
        if(this.extends.some( item=>item.isDeprecated(name))){
            return true;
        }
        if(this.implements.some( item=>item.isDeprecated(name))){
            return true;
        }
        return false;
    }
    ...
    ckeckAllDescriptors(){
        this.descriptors.forEach( (list, name)=>{
            list.forEach(item=>{
                if(item.isRemoved){
                    this.removedNamedDescriptors[name] = true;
                }
                if(item.isDeprecated){
                    this.deprecatedNamedDescriptors[name] = true;
                }
            });
