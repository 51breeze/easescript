...
class Module extends Type{
    ...
    get isDeclaratorModule(){
        ...
        return this[symbolKey].isDeclaratorModule = this.getStacks().every( stack=>{
    ...
    get isLocalModule(){
        ...
        if(value != null)return value;
        return this[symbolKey].isLocalModule = this.getStacks().some( stack=>{
    ...
    dynamicAttribute(propertyType, context=null){
        ...
        if( this.implements ){
            for(const impl of this.implements ){
                const result = impl.dynamicAttribute( propertyType, context);
                if( result ){
                    return result;
                }
            }
    ...
    addMember(name, desc, flag=false){
        if(desc && desc.isConstructor){
            ...
            const isStatic = !!(desc.static || desc.isReadonly || this.static);
            ...
            const elements = declareGenerics.map( item=>{
                if(options.fetchDeclareGenericsDefaultValue && options.inbuild){
                    if(item.type().assignType){
                        return item.type().assignType.type().toString({}, options); 
                    }
                    return 'any';
                }
