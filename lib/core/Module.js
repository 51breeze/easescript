...
class Module extends Type{
    ...
    constructor( compilation ){
        ...
        this[symbolKey] = {
            stacks:[],
            stack:null,
            removedDescriptors:Object.create(null)
        };
    ...
    pushRemovedDescriptor(name, descriptor){
        const dataset = this[symbolKey].removedDescriptors;
        const list = dataset[name] || (dataset[name]=[]);
        if(!list.includes(descriptor)){
            list.push(descriptor);
        }
    }

    checkDescriptorRemoved(name, descriptor){
        const dataset = this[symbolKey].removedDescriptors;
        if(dataset[name]){
            return dataset[name].some( item=>{
                if(descriptor.isMethodGetterDefinition){
                    return item.isMethodGetterDefinition
                }else if(descriptor.isMethodSetterDefinition){
                    return item.isMethodSetterDefinition
                }else if(descriptor.isMethodDefinition){
                    return item.isMethodDefinition
                }else if(descriptor.isPropertyDefinition){
                    return item.isPropertyDefinition
                }
            });
        }
        if(this.extends.some( item=>item.checkDescriptorRemoved(name,descriptor))){
            return true;
        }
        if(this.implements.some( item=>item.checkDescriptorRemoved(name,descriptor))){
            return true;
        }
        return false;
    }
    ...
    addDescriptor(name, descriptor){
        if(descriptor.isRemoved){
           this.pushRemovedDescriptor(name, descriptor)
        }
    ...
    checkDescriptors(name){
        ...
        if( dataset && dataset.length > 1 ){
            ...
            const result = dataset.every( descriptor=>{
                const token = descriptor.toString();
                const has = dataset.some( item=>{
                    ...
                    if( item.isPropertyDefinition ){
                        if( token ==='PropertyDefinition' ){
                            const oldT = item.type();
                            const newT = descriptor.type();
                            if( oldT && newT ){
                                return push(descriptor, Utils.checkTypeForBoth(oldT.type(), newT.type()));
                            }
                        }
                    }else if( item.isMethodGetterDefinition ){
                        if( token ==='MethodGetterDefinition' ){
                            const oldT = item.inferReturnType();
                            const newT = descriptor.inferReturnType();
                            if( oldT && newT ){
                                return push(descriptor, Utils.checkTypeForBoth(oldT.type(), newT.type()));
                            }
                        }
                        ...
                        if( token ==='MethodSetterDefinition' ){
                            const oldP = item.params[0];
                            const newP = descriptor.params[0];
                            if( oldP && newP ){
                                return push(descriptor, Utils.checkTypeForBoth(oldP.type(), newP.type()));
                            }
                        }
                    }else if( item.isMethodDefinition ){
                        if( descriptor.isMethodDefinition ){
                            const oldP = item.params;
                            const newP = descriptor.params;
                            if(oldP && newP && oldP.length === newP.length){
                                return push(descriptor,oldP.every( (oldT,index)=>{
    ...
    ckeckAllDescriptors(){
        Array.from(this.descriptors.keys()).forEach( name=>{
            const [result, sameitems] = this.checkDescriptors(name);
            if( !result ){
    ...
    clear( compilation ){
        ...
        this[symbolKey].removedDescriptors = Object.create(null);

        if( compilation && this.compilation !== compilation ){
            const dependencies = this.dependencies;
            const clearMember=(target)=>{
                for(let key in target){
                    const desc = target[key];
                    if( !desc.isStack && desc.isAccessor ){
                        if( desc.get && desc.get.compilation === compilation ){
                            dependencies.delete(key);
                            delete desc.get;
                        }
                        if( desc.set && desc.set.compilation === compilation ){
                            dependencies.delete(key);
                            delete desc.set;
                        }
                    ...
                    }else if( desc.isStack && desc.compilation === compilation){
                        delete target[key];
                        dependencies.delete(key);
                    } 
                }
            }
            clearMember(this.methods);
            clearMember(this.members);
        }else{
    ...
    addAsset(resolve,file,content,type,assign){
        if( !this.assets.has(resolve) ){
            this.assets.set(resolve, {file,resolve,content,type,assign,id:this.getName()} );
            return true;
        }
        return false;
    }
    ...
    addRequire(key,name,from,resolve,extract,stack){
        if( !this.requires.has(name) ){
            this.requires.set(name,{key,name,from,resolve,extract,stack,id:this.getName()});
            return true;
        }
        return false;
    }
    ...
    toString( context, options={}){
        ...
        if( stackModule && declareGenerics && declareGenerics.length>0){
            ...
            const elements = declareGenerics.map( item=>{
                ...
                if(options.complete){
                    return type.toString(null, options);
                }else{
