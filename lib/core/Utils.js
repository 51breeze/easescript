...
const structTableMappingTypeName={
    'tinyint':'int',
    'smallint':'int',
    'mediumint':'int',
    'int':'int',
...
const Utils ={
    ...
    checkTypeForBoth(left,right, isStrict=true, context=null){
        ...
        if(left.isAnyType){
            ...
            return left.elements.every( (item,index)=>{
                ...
                return this.checkTypeForBoth(item.type(), right.elements[index].type(),isStrict,context);
            });
            ...
            if(this.checkTypeForBoth(left.inherit.type(), right.inherit.type(),isStrict,context)){
                return left.generics.every( (item,index)=>{
                    ...
                    return this.checkTypeForBoth(item.type(), right.generics[index].type(),isStrict,context);
                });
            }
            return false;
            ...
            return lItems.every(item=>{
                const type = item.type();
                return rItems.some(item=>this.checkTypeForBoth(type, item.type(),isStrict, context))
            });
    ...
    getOriginType(type, exclude=null){
        ...
        let origin = type;
        if( type.isClassGenericType && type.isClassType && type.types[0]){
        ...
        return this.isModule(type) ? type.type() : (type || origin);
    },
    ...
    isEqualModule(left, right){
        if(!Utils.isModule(left) || !Utils.isModule(right))return false;
        left = left.type();
        right = right.type();
        return left === right;
    },
    extractArrayPatternItem(type, index){
        if(type){
            if(type.isLiteralArrayType || type.isTupleType){
                ...
                return Utils.extractArrayPatternItem(type.left.type(), index) || Utils.extractArrayPatternItem(type.right.type(), index);
            }
        }
        return null;
    },
