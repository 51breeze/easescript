...
module.exports={
    ...
    isLocalModule( module ){
        return module && module.isLocalModule && this.isTypeModule(module);
    },
    ...
    checkTypeForBoth(left,right){
        ...
        if( left.isLiteralObjectType || 
            ...
            if(!lReturnType && rReturnType)return false;
            if(!this.checkTypeForBoth(lReturnType, rReturnType))return false;
            return lParams.every( (item,index)=>{
                if(!rParams[index])return false;
                return this.checkTypeForBoth(item.type(), rParams[index].type());
            });
            ...
            return left.elements.every( (item,index)=>{
                ...
                return this.checkTypeForBoth(item.type(), right.elements[index].type());
            });
            ...
            if(this.checkTypeForBoth(left.inherit.type(), right.inherit.type())){
                return left.generics.every( (item,index)=>{
                    if(!right.generics[index])return false;
                    return this.checkTypeForBoth(item.type(), right.generics[index].type());
                });
            }
            return false;
            ...
            return this.checkTypeForBoth(left.left.type(), right.left.type()) && this.checkTypeForBoth(left.right.type(), right.right.type());
        }else if( left.isAliasType ){
            if(!right.isAliasType)return false;
            return this.checkTypeForBoth(left.inherit.type(),right.inherit.type());
        }else if( left.isKeyofType ){
            if( !right.isKeyofType)return false;
            return this.checkTypeForBoth(left.referenceType.type(),right.referenceType.type());    
