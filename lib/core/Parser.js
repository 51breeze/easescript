...
tokTypes._enum.updateContext = tokTypes._interface.updateContext = tokTypes._class.updateContext;
...
class SyntaxParser extends JSXParser {
    ...
    constructor(options, input, startPos){
        ...
        this.testTokens = [];
        this.stackComments=[];
        this.keywords =  new RegExp( this.keywords.source.replace(")$","|is|package|implements|static|final|public|internal|protected|private|when|then|enum|interface|abstract)$") );
    ...
    parseVarStatement(node, kind, flag, bracketStarted, isDeclareInterface){
        this.getComments(node);
        if( flag ){
            if(!bracketStarted) this.next();
            if( this.eat( tokTypes.bracketL ) ){
                ...
                if( bracketStarted && isDeclareInterface ){
                    ...
                    if( genericity || this.type=== tokTypes.parenL ){
                        decl.key = id;
                ...
                this.expect( tokTypes.colon )
                decl.id = id;
                decl.init = this.parseTypeStatement();
                this.semicolon();
    ...
    parseBlock(createNewLexicalScope, node){
        ...
        const body = super.parseBlock(createNewLexicalScope, node);
        this.stackComments.length = 0;
        return body;
    }
    ...
    parseBindingAtom(){
        ...
        if( node.question || node.acceptType ){
    ...
    parseStatement(context, topLevel, exports, isDeclaredModule=false){
        ...
        if( topLevel && this.value ==="struct"){
            const type = this.step();
            if( type === tokTypes.name && this.value ==='table' ){
                this.apply();
    ...
    parseDeclarator(node){
        ...
        if( this.type === tokTypes._const || this.type === tokTypes._var ){
            ...
            node.expression = this.parseVarStatement(node, "var", true);
            ...
            node.kind = "module";
            return this.parseDeclareModule(node);
        }
    ...
    parseTypeStatement( prefix, exclude){
        let start = this.start, startLoc= this.startLoc;
        ...
        if( this.type  === tokTypes._void ){
        ...
        }else if( tokTypes._typeof === this.type ){
            this.next();
            ...
            if( this.value ==="keyof"){
                ...
                return this.finishNode(node, 'TypeKeyofDefinition');
            }
            node.value = this.parseChainIdentifier();
        ...
        while( true ){
            if(exclude !== tokTypes.bitwiseOR && this.eat(tokTypes.bitwiseOR) ){
                const typeItem = this.parseTypeStatement(null,tokTypes.bitwiseOR)
                if( typeItem.type ==="TypeUnionDefinition"){
                    unions.push( ...typeItem.elements );
                }else{
                    unions.push( typeItem );
                }
            }else if(exclude !== tokTypes.bitwiseAND && this.eat(tokTypes.bitwiseAND)){
                const left = unions.pop()
                const intersection = this.startNodeAt(left.start,_startLoc);
                intersection.left = left;
                intersection.right = this.parseTypeStatement(null, tokTypes.bitwiseAND);
    ...
    parseClassProperty(node,kind,bracketStarted,isDeclareInterface) {
        node = this.parseVarStatement( node, kind, true, bracketStarted, isDeclareInterface);
    ...
    parseClassElement( constructorAllowsSuper, isDeclareInterface, isUseStatement){
        ...
        const element = isProperty ? 
                    this.parseClassProperty( this.startNode(), isBracket ? 'var' : this.value, isBracket , isDeclareInterface || isUseStatement ) : 
    ...
    parseImportSpecifiers() {
        ...
        if (this.type === tokTypes.name) {
            ...
            var node = this.startNode();
            node.local = this.parseChainIdentifier();
