...
class SyntaxParser extends JSXParser {
    ...
    parseStructTableDeclarator(node=null){
        ...
        const parseNode=( isIdent )=>{
            ...
            return this.type === tokTypes.name || this.type.keyword? this.parseIdent(true) : this.parseLiteral(this.value);
        };
        const parseToken=()=>{
    ...
    parseEnumProperty(isDeclare){
        ...
        if( this.eat(tokTypes.eq) ){
            node.init = this.type === tokTypes.num || this.type === tokTypes.string ? this.parseLiteral(this.value) : this.parseExprAtom();
        }
