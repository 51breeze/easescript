...
class SyntaxParser extends JSXParser {
    ...
    readToken( code ){
        ...
        if(code===60 && (this.disabledJSXExpressionFlag || this.__parseTypeStatement)){
        ...
        if( this.type === tokTypes.relational && this.value && this.value.charCodeAt(0) === 60 ){
            ...
            do{
                ...
                if( this.eat(tokTypes.eq) ){
                    ...
                    if( this.eat(tokTypes._extends) ){
                        left.extends = this.parseTypeDefinition();
                    }
    ...
    parseTypeDefinitionItem(prefix){
        ...
        if( tokTypes._typeof === this.type){
            this.next();
            node.value = this.parseExprSubscripts();
        ...
        if( this.type  === tokTypes._void ){
            ...
            if( this.value ==="keyof"){
                ...
                node.value = this.parseChainIdentifier();
                if(node.value.type ==='Identifier' && this.type === tokTypes._is){
                    this.next();
            ...
            const parseProperty = ()=>{
                ...
                if(propHash[prop.key.name] === true){
    ...
    parseVarStatementDefinition(node, kind,  bracketStarted, isDeclareInterface){
        ...
        if(dynamic){
            if( this.eat(tokTypes.colon)){
                ...
                id = this.parseSubscripts(id, id.start, id.loc)
            }
            this.expect( tokTypes.bracketR )
        }
        if(bracketStarted){
    ...
    parseClassElement( constructorAllowsSuper, isDeclareInterface, isUseStatement){
        ...
        const readonly = this.type === tokTypes.name && this.value ==='readonly'
        const isProperty = isBracket || this.type === tokTypes._const || this.type === tokTypes._var || readonly;
    ...
    parseExport(node, exports) {
        this.next();
        if(this.eat(tokTypes.eq)) {
            node.expression = this.parseIdent();
            this.semicolon();
