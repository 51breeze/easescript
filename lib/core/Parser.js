...
class SyntaxParser extends JSXParser {
    ...
    shouldParseExportStatement(){
        if( this.parseDeclareModuleContext ){
            if(this.type === tokTypes.name && (this.value ==="declare" || this.value ==="type") || this.type.keyword==='interface'){
                return true;
            }
        }
            ...
            while( !this.eat(tokTypes.braceR) ){
                ...
                if( item ){
                    ...
                    if( this.type === tokTypes._export ){
                        node.body.push(this.parseExport(this.startNode(), exports))
                        ...
                        if(this.value ==="namespace"){
                            this.next();
                            const ns = this.startNode();
                            ns.body = [];
                            if(tokTypes.braceL !== this.type){
                                ns.id = this.parseExprSubscripts();
                            }
                            this.expect(tokTypes.braceL)
                            while(!this.eat(tokTypes.braceR)){
                                const item = metatype();
                                if( item ){
                                    ...
                                    if( this.type === tokTypes._export ){
                                        ns.body.push(this.parseExport(this.startNode(), exports))
                                    }else if( this.type === tokTypes._import ){
                                        ns.body.push(this.parseImport(this.startNode()))
                                    }
                                    else{
                                        if(this.value ==="declare"){
                                            this.next();
                                        }
                                        ns.body.push( this.parseDeclarator(this.startNode(), true) );
                                    }
                                }
                            }
                            this.finishNode(ns,  "NamespaceDeclaration");
                            node.body.push( ns );
                        }else{
                            if(this.value ==="declare"){
                                this.next();
                            }
                            node.body.push( this.parseDeclarator(this.startNode(), true) );
                        }
                    }
                }
            }
