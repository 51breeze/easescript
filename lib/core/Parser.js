...
tokTypes.braceL.updateContext = function(prevType) {
    const isStat  = this.curContext() === tc_type_statement;
    this.context.push(isStat || this.braceIsBlock(prevType) ? tokContexts.b_stat : tokContexts.b_expr);
    this.exprAllowed = !isStat;
};
...
class SyntaxParser extends JSXParser {
    ...
    popContext(){
        const hasPop = this.type === tokTypes.braceR || this.type === tokTypes.parenR;
    ...
    popFunctionContext(){
        switch(this.type){
           ...
           // case tokTypes._package   :
            case tokTypes._struct    :
            {
                if(this.curContext() === tokContexts.f_stat){
                    this.context.pop()
                }
            }
        }
        ...
        if( this.type === tokTypes.relational && this.value && this.value.charCodeAt(0) === 60 ){
            ...
            if( flag ){
                ...
                if( this.type === tokTypes.relational && this.value.charCodeAt(0) === 62 && this.value.length===1 ){
                    ...
                    console.log( this.type, this.start )
    ...
    updateContext(prevType) {
        ...
        let pos = this.context.length-1;
        let last = this.context[pos];
        if(this.exprAllowed && (last=== tc_type_statement || last === tokContexts.b_stat && this.context[pos-1] === tc_type_statement) ){
            this.exprAllowed = false
