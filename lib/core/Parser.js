...
class SyntaxParser extends JSXParser {
    ...
    raiseRecoverable(pos, message){
        ...
        if(onError){
            if(message.endsWith('has already been declared')){
                //onError(message,  this.endLoc.line, this.endLoc.column -  (this.start - pos), pos)
                return;
            }
        }
    ...
    parseParenAndDistinguishExpression(canBeArrow) {
        ...
        if( val.type !=="SequenceExpression" && !this.canInsertSemicolon() && (this.type === tokTypes._this || this.type === tokTypes.name ) ){
            ...
            node.value = val;
            this.finishNode(node, "TypeDefinition");
        ...
        if( this.eat(tokTypes._extends) ){
            ...
            do{ 
                ...
                if( _extends ){
                    ...
                    node.extends.push(_extends);
                }
            }while( this.eat(tokTypes.comma) );
            node.superClass = node.extends[0];
        }
   ...
   parseBindingList(close, allowEmpty, allowTrailingComma) {
    ...
    parseBindingAtom(){
        ...
        const node = this.type === tokTypes._this ? this.parseExprAtom() : super.parseBindingAtom();
   ...
   parseExprList(close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
            ...
            if(refDestructuringErrors && tokTypes.parenR === close && elt){
                if( (elt.type==='Identifier'||elt.type==='SpreadElement') && this.eat(tokTypes.colon)){
                    ...
                    if(this.eat(tokTypes.eq)){
                        const node = this.startNodeAt(elt.start, elt.loc?.start);
                        node.left = elt;
    ...
    parseStatement(context, topLevel, exports, isDeclaredModule=false){
            ...
            case tokTypes._annotation:
                if( topLevel ) {
                    ...
                    if(this.type === tokTypes._interface){
                        node = this.parseStatement(context, topLevel, exports, isDeclaredModule);
                        node.decorator = true;
                        return node;
                    }
                    node.name = this.value;
                    node.id = this.parseChainIdentifier();
                    return this.parseAnnotation(false, node);
                }
            break;
        ...
        if( topLevel && this.value ==="declare" ){
            ...
            if(this.type === tokTypes._abstract){
                abstract = this.parseIdent(true);
            }else if(this.type===tokTypes._annotation){
                this.next();
                if(this.type ===tokTypes._interface){
                    let node = this.parseStatement(context, topLevel, exports, true);
                    node.decorator = true;
                    return node;
                }else{
                    this.unexpected();
            ...
            let node =  this.startNode();
            node.abstract = abstract;
            return this.parseDeclarator(node);
        }
    ...
    parseStructTableDeclarator(node=null){
        this.disabledJSXExpressionFlag = true;
        this.next();
        ...
        if( this.eat(tokTypes._extends) ){
            do{ 
                node.extends.push( this.parseChainIdentifier() );
            }while( this.eat(tokTypes.comma) );
        }
        ...
        const parseToken=()=>{
            ...
            let value = parseNode();
            if( value.type ==="Identifier" || value.type ==='MemberExpression' || value.type==='Literal' && value.isBackQuote){
        ...
        while( tokTypes.braceR !== this.type ){
            if(this.type === tokTypes._annotation){
                node.body.push(this.parseAnnotation());
            }
    ...
    shouldParseExportStatement(){
        if( this.parseDeclareModuleContext ){
            if(this.type === tokTypes.name && (this.value ==="declare" || this.value ==="type" || this.value ==="namespace") || 
                this.type.keyword==='interface' || 
                this.type.keyword ==='abstract' ||
                this.type.keyword ==='public' ||
                this.type.keyword ==='internal' ||
                this.type.keyword ==='protected' ||
                this.type.keyword ==='struct' ||
                this.type.keyword ==='enum' ||
                this.type.keyword ==='static'
            ){
                return true;
            }
        }
    ...
    parseWhenStatement(node){
        ...
        const parseCondition=(node=null)=>{
            if(tokTypes.parenR !== this.type){
                let startNode = this.startNode();
                let test = node || this.parseExprOps(true);
    ...
    parseEnumProperty(isDeclare){
        ...
        if (this.type === tokTypes.name || this.type.keyword) {
            ...
            node.key.name = this.type === tokTypes.name ? this.value : this.type.keyword;
        ...
        if( this.eat(tokTypes.eq) ){
            if(this.type === tokTypes.plusMin){
                node.init = this.parseLiteralMinus()  
            }else{
                node.init =  this.parseExprOps(true);
            }
        }
        this.finishNode(node, "EnumProperty");
    ...
    parseLiteralMinus() {
        ...
        if(this.type === tokTypes.num){
            let value = minus ? -this.value : this.value;
            node.value = value;
            node.raw = this.input.slice(start, this.end);
    ...
    parseEnumStatement(node, isDeclare=false){
        this.disabledJSXExpressionFlag = true;
        this.next();
        node.key = this.parseIdent(false)
    ...
    parseDeclaratorDone(node, semi=false){
        this.disabledJSXExpressionFlag = false;
        this.parseDeclaratorContext = false;
        if(semi)this.semicolon();
        return node
    }
    ...
    parseDeclarator(node, isDeclare=true){
        ...
        if( this.type === tokTypes._const || this.type === tokTypes._var || this.isLet()){
            ...
            return this.parseDeclaratorDone(this.finishNode(node,"DeclaratorVariable"), true);
            ...
            return this.parseDeclaratorDone(this.finishNode(node, "DeclaratorFunction"), true);
            ...
            node.kind = "enum";
            return this.parseDeclaratorDone(this.parseEnumStatement(node, isDeclare))
            ...
            node.kind = "module";
            return this.parseDeclaratorDone(this.parseDeclareModule(node));
        ...
        if( this.type !== tokTypes.braceL && node.id && node.id.name ==="type" ){
            if(!isDeclare){
                return this.parseDeclaratorDone(this.parseTypeStatement(node));
            }
            ...
            return this.parseDeclaratorDone(this.finishNode(node,"DeclaratorTypeAlias"), true);
        }
        ...
        while( index < content.length ){
            ...
            if( comment === false && code === 47 && content.charCodeAt( index+1 ) === 42){
                ...
                //||
                    ...
                    const allow = (code >= 97 && code <= 122) || 
                        (code >= 65 && code <= 90) || 
                        (code >= 48 && code <= 57) ||
                        code===44 || code===9 || code===13 || code===10 || code===36 || code===32 || 
                        code===95 || code===46 || code===92 || code===58 || code===38 || code===124 || code===61 || code===63
    ...
    parseTypeDefinition(prefix, isRoot=true, isGeneric=false){
        ...
        let start = this.start, startLoc= this.startLoc;
        let node = this.parseTypeDefinitionItem(prefix, readonly);
        ...
        if(unions.length > 1){
            ...
            node = this.finishNode(unionNode,"TypeUnionDefinition");
        }else{
            node = unions[0];
        }
        node.readonly = !!readonly;

        if(!isGeneric && tokTypes._extends===this.type && (node.type==="TypeDefinition" && node.value.type ==='Identifier' || node.type==="TypeInferDefinition")){
            const make = (_name, start, startLoc)=>{
                if(!_name){
                    _name = this.parseTypeDefinition();
                }
                if(this.eat(tokTypes._extends)){
                    let infer = _name.type ==='TypeInferDefinition';
                    let conditional = this.startNodeAt(start,startLoc);
                    let condition = this.startNodeAt(start,startLoc);
            ...
            return make(node,start, startLoc)
        }
        return node;
    }
    ...
    parseTypeDefinitionItem(prefix, readonlyNode=null){
        ...
        if( this.type  === tokTypes._void ){
            ...
            if( this.value ==="keyof"){
                ...
                node.value = this.parseChainIdentifier();
                if(node.value.name === 'infer' && this.type === tokTypes.name){
                    delete node.value;
                    node.argument = this.parseIdent();
                    typeName="TypeInferDefinition"
                    //this.finishNode(node, 'TypeInferDefinition')
                }else{
            ...
            node.value = this.parseLiteralMinus();
            this.finishNode(node, typeName);
        }
        else{
            if(readonlyNode){
    ...
    parseAnnotation(nonStatement=false, node=null, inline=false){
        if(!node){
            node = this.startNode();
            this.next();
            node.name = this.value;
    ...
    parseStatementAnnotation(inline=false){
        if(this.type === tokTypes._annotation){
            ...
            if(!inline && this.type === tokTypes._interface){
                node = this.parseStatement(null,true);
                node.decorator = true;
                return node;
            }
            node.name = this.value;
            node.id = this.parseChainIdentifier();
            return this.parseAnnotation(false, node, inline);
        }
        return null;
    }
    ...
    parsePackage(node){
        ...
        if( this.eat(tokTypes.braceL) ){
            node.isBlock = true;
            while( !this.eat(tokTypes.braceR) ){
                const item = this.parseStatementAnnotation();
                if( item ){
                    node.body.push(item);
                }else{
