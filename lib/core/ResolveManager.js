...
class ResolveManager{
    ...
    init(){
        let resolveFolders = [
            this.#comppiler.getWorkspaceFolders().map(dir=>path.dirname(dir)),
            this.#comppiler.getWorkspaceFolders(), 
        ...
        this.#resolveFolders = Array.from((new Set(resolveFolders)).values());
        this.#cache = Object.create(null);
        this.#cache.resolve = Object.create(null);
        this.#nodeModuleFolders = this.#comppiler.getNodeModuleFolders()
    }
    ...
    resolve(file, context=null, requireFlag=false, allowDir=true, dependency=true){
        ...
        if(!value && dependency){
            for(let folder of this.#nodeModuleFolders ){
                if(context && !path.normalize(context).startsWith(path.normalize(path.dirname(folder)))){
                    continue;  
                }
    ...
    _resolveFile(file, addSuffix=false, allowDir=false){
        ...
        if(addSuffix){
            for(let ext of this.#extensions){
                let result = this._resolveFile(file+ext, false, allowDir)
                if(result){
                    return result
                }
            }
