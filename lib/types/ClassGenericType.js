...
class ClassGenericType extends Type{
    ...
    check(stack, context){
        ...
        if(!type.isClassGenericType){
            ...
            if(isWrap){
                return this.getWrapCheckerType(inherit).check(stack, context);
            }
            if( this.hasDynamicAttribute(inherit) ){
                return Namespace.globals.get('object').is( type );
            }
            return this.isClassType ? this.types[0].is(type,context) : inherit.check(stack, context);
        }
    ...
    hasDynamicAttribute(object){
        if(object && object.isModule && (object.isInterface || object.isClass)){
            return !!object.dynamicAttribute( Namespace.globals.get('string') )
        }
        return false;
    }
    ...
    is( type, context, options={}){
        ...
        if( !this.isClassType ){
            ...
            const assign = type.types[0];
            const accept = this.types[0];
            if(accept){
                return assign ? accept.type().is(assign.type(), context, options) : false;
            }
            return true;
        }
    }
