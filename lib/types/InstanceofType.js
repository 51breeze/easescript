...
class InstanceofType extends Type{
    constructor(inherit,target,generics=[], isThis=false, newModuleType=null){
        ...
        this.hasGenericType = this.inherit.type().hasGenericType || this.generics.some( type=>!!(type && type.hasGenericType) );
    }
    clone( inference ){
        if( !inference || !this.hasGenericType ){
            return this;
        }
        return new InstanceofType(Utils.getOriginType(this.inherit.type().clone(inference)), this.target, this.generics.map( type=>type.clone(inference) ), this.isThisType );
    }
    getTypeKeys(){
    ...
    toString(context,options={}){
        ...
        if(inherit.isAliasType && this.generics.length > 0){
            ...
            return `${inherit.toString(context, {onlyTypeName:true})}<${this.generics.map( type=>type.toString(context,options)).join(', ')}>`;
        }
