...
const InstanceofType = require("./InstanceofType.js");
...
class FunctionType extends Type{
    ...
    getInferReturnType(){
        ...
        if( !r && this.target ){
            if(this.target.isFunctionExpression){
                const value = this.target.inferReturnType();
                //防止递归死循环
                if(this.target.async){
                    ...
                    if(!promiseType.is(origin)){
                        return this.target.__asyncType = new InstanceofType(promiseType, null, [value]);
                    }
                }
                return value;
            }else{
    ...
    toString(context={}, options={}){
        ...
        const params = _params.map( (item,index)=>{
            if( item.isObjectPattern ){
                ...
                const properties = item.elements.map( property=>{
                    if( property.isAssignmentPattern ){
                        ...
                        return `${property.left.value()}:${acceptType.toString(ctx,options)} = ${property.right.raw()}`;
                    }
                ...
                const type = _params[index].type();
                let ctx = type.isClassGenericType || Utils.isModule(type) ? null : context
                if(hasNested(type)){
                    ctx = {}
                }
                const name = item.isThisExpression ? 'this' : item.value();
