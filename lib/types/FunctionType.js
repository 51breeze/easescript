...
class FunctionType extends Type{
    ...
    is(type, context,options={}){
        ...
        if( type.params.length > 0 ){
            ...
            const result = type.params.every( (item,index)=>{
                ...
                if( item.isObjectPattern || acceptType && acceptType.isObjectPattern ){
                    return true;
                }
                ...
                return acceptType && acceptType.type().is(item.type(), context, options);
            });
            if( !result ){
                return false;
            }
    ...
    toString(context={}, options={}){
        ...
        let complete = false;
        let has = options.chain.some( item=>{
            return item.isLiteralObjectType || item.isLiteralArrayType || item.isUnionType || item.isTupleType || item.isIntersectionType || item.isComputeType;
        })

        if( options.hasExists || has || !(context.stack && (context.stack.isCallExpression || context.stack.isNewExpression))){
            context = {};
            complete = true;
        }
